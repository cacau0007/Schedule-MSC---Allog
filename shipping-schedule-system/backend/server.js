        ...
        if (data.results.length > 0) {
            ...
            for (let i = 0; i < Math.min(data.results.length, 6); i++) {
                ...
                try {
                    ...
                    // PASSO 3: Encontrar TODAS as linhas de resultado (headers apenas)
                    const allRows = await page.evaluate(() => {
                        const rows = [];
                        document.querySelectorAll('div, article, section').forEach(el => {
                            const text = el.innerText || '';
                            const rect = el.getBoundingClientRect();
                            // Linha de header tem altura entre 50-200px e contém Days + tipo de rota
                            // NÃO depender de "MSC" pois alguns navios não têm (ex: ESPIRITO SANTO EXPRESS)
                            if (text.includes('Days') &&
                                (text.includes('Transhipment') || text.includes('Direct')) &&
                                rect.width > 800 && rect.height > 50 && rect.height < 200) {
                                rows.push({ 
                        ...
                        for (const row of rows) {
                            if (Math.abs(row.top - lastTop) > 40) {
                                uniqueRows.push(row);
                                lastTop = row.top;
                            }
                        }
                        return uniqueRows.map((r, idx) => ({ index: idx, top: r.top, text: r.text }));
                    });
                    ...
                    // PASSO 6: Clicar na linha correta
                    const clicked = await page.evaluate((targetIndex, expectedText) => {
                        const rows = [];
                        document.querySelectorAll('div, article, section').forEach(el => {
                            const text = el.innerText || '';
                            const rect = el.getBoundingClientRect();
                        ...
                        // Encontrar a linha pelo texto esperado
                        let targetRow = null;
                        for (const row of uniqueRows) {
                            // Comparar parte do texto
                            const expectedPart = expectedText.substring(0, 40);
                        ...
                        // Se não encontrou pelo texto, pegar pelo índice relativo
                        if (!targetRow) {
                            // Pegar a linha que está mais próxima do topo (primeira visível)
                            if (uniqueRows.length > 0) {
                                targetRow = uniqueRows[0];
                            }
                        }
                        
                        if (!targetRow) {
                            return { success: false, error: 'Nenhuma linha encontrada', rowsFound: rows.length };
                        }
                        ...
                        return { success: true, rowText: targetRow.text.substring(0, 60), rowTop: targetRow.top };
                    }, i, allRows[i].text);
                    ...
                    if (clicked && clicked.success) {
                        ...
                        await page.evaluate((index) => {
                            // Procurar e clicar no botão de fechar (chevron) ou na linha
                            const expandedPanels = document.querySelectorAll('[aria-expanded="true"]');
                            expandedPanels.forEach(el => {
                                const btn = el.querySelector('button, svg');
                                if (btn) btn.click();
                                else el.click();
                            });
                            
                            // Também tentar fechar clicando nas linhas
                            const rows = [];
                            document.querySelectorAll('div, article, section').forEach(el => {
                                const text = el.innerText || '';
                                const rect = el.getBoundingClientRect();
