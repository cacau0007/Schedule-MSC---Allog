...
app.post('/api/schedules', async (req, res) => {
    const { pol, pod, carrier, carriers, service } = req.body;
    
    // Aceitar tanto 'carrier' (string) quanto 'carriers' (array)
    let carrierList = carriers || (carrier ? [carrier.toUpperCase()] : ['MSC']);
    if (typeof carrierList === 'string') {
        carrierList = [carrierList.toUpperCase()];
    }
...
app.post('/api/export', async (req, res) => {
    ...
    try {
        ...
        const servicesText = services.length > 0 ? services.join(', ') : 'ALL';
        
        // Texto do header: "ROTA: Shanghai x Navegantes | SERVIÇO: Ipanema"
        const headerText = `${pol} x ${pod} | MSC ${servicesText}`;
        
        // LINHA 1: Header amarelo com rota e serviço (ocupa todas as colunas)
        ...
        const headers = ['VESSEL', `ETD ${pol}`, 'TRANS.', 'ETA TRANS.', `ETA ${pod}`, 'TRANSIT', 'SERVICO'];
    ...
    try {
        ...
        if (data.results.length > 0) {
            ...
            for (let i = 0; i < Math.min(data.results.length, 6); i++) {
                ...
                try {
                    // PASSO 1: FECHAR TODOS OS PAINÉIS ABERTOS (muito importante!)
                    await page.evaluate(() => {
                        // Clicar em todos os botões de fechar/expandir que estão abertos
                        document.querySelectorAll('[aria-expanded="true"], [class*="expanded"], [class*="open"]').forEach(el => {
                            const btn = el.querySelector('button, svg, [class*="chevron"]');
                            if (btn) btn.click();
                        });
                        
                        // Também clicar nas linhas que podem estar expandidas
                        document.querySelectorAll('div, article, section').forEach(el => {
                            const text = el.innerText || '';
                            const rect = el.getBoundingClientRect();
                            // Se é uma linha de resultado (header) que está expandida
                            // NÃO depender de "MSC" no texto
                            if (text.includes('Days') &&
                                (text.includes('Transhipment') || text.includes('Direct')) &&
                                rect.height > 200) { // Expandido = altura maior
                                const btn = el.querySelector('button, svg');
                                if (btn) btn.click();
                            }
                        });
                    });
                    ...
                    // PASSO 2: Scroll para o topo da página
                    await page.evaluate(() => {
                        window.scrollTo(0, 0);
                    });
                    await new Promise(r => setTimeout(r, 500));
                    
                    // PASSO 3: DEBUG - Encontrar elementos clicáveis na página
                    const debugInfo = await page.evaluate(() => {
                        ...
                        // Procurar TODOS os elementos que contêm "Days"
                        document.querySelectorAll('*').forEach(el => {
                            const text = el.innerText || '';
                            const rect = el.getBoundingClientRect();
                            
                            if (text.includes('Days') && rect.width > 100 && rect.height > 20) {
                                debug.daysElements.push({
                                    tag: el.tagName,
                        ...
                        // Procurar elementos que parecem ser linhas de resultado
                        // NOVA ABORDAGEM: procurar por botões/chevrons próximos a "Days"
                        document.querySelectorAll('button, [role="button"], svg, [class*="chevron"], [class*="expand"], [class*="arrow"]').forEach(el => {
                            ...
                            if (text.includes('Days') && rect.top > 0 && rect.top < 2000) {
                                debug.clickableRows.push({
                                    tag: el.tagName,
                    ...
                    console.log(`      DEBUG - Elementos com "Days": ${debugInfo.daysElements.length}`);
                    if (debugInfo.daysElements.length > 0 && debugInfo.daysElements.length <= 10) {
                        debugInfo.daysElements.forEach((el, idx) => {
                            console.log(`        [${idx}] ${el.tag} ${el.width}x${el.height} top=${el.top}: "${el.text.substring(0, 50)}..."`);
                        });
                    }
                    console.log(`      DEBUG - Elementos clicáveis: ${debugInfo.clickableRows.length}`);
                    if (debugInfo.clickableRows.length > 0 && debugInfo.clickableRows.length <= 10) {
                        debugInfo.clickableRows.forEach((el, idx) => {
                            console.log(`        [${idx}] ${el.tag}>${el.parentTag} ${el.width}x${el.height} top=${el.top}: "${el.text.substring(0, 40)}..."`);
                        });
                    }
                    
                    // PASSO 3B: Encontrar linhas de resultado usando NOVA lógica
                    // Procurar por elementos que contenham o nome do navio atual
                    const allRows = await page.evaluate((vesselName) => {
                        const rows = [];
                        const vesselParts = vesselName.toUpperCase().split(' ');
                        const vesselKey = vesselParts.length > 1 ? vesselParts.slice(0, 2).join(' ') : vesselParts[0];
                        
                        // Abordagem 1: Procurar por linhas com "Days" + "Direct/Transhipment"
                        document.querySelectorAll('div, article, section').forEach(el => {
                            const text = el.innerText || '';
                            const rect = el.getBoundingClientRect();
                            
                            // Critérios mais flexíveis
                            const hasDays = text.includes('Days') || text.includes('days');
                            const hasRouteType = text.includes('Transhipment') || text.includes('Direct');
                            const hasVessel = text.toUpperCase().includes('MSC') || 
                                             text.toUpperCase().includes('EXPRESS') ||
                                             text.toUpperCase().includes(vesselKey);
                            
                            // Critérios de tamanho mais flexíveis
                            const goodSize = rect.width > 300 && rect.height > 30 && rect.height < 300;
                            const isVisible = rect.top > -500 && rect.top < 3000;
                            
                            if (hasDays && (hasRouteType || hasVessel) && goodSize && isVisible) {
                                rows.push({ 
                        ...
                        // Remover duplicatas (elementos muito próximos)
                        const uniqueRows = [];
                        let lastTop = -100;
                        for (const row of rows) {
                            if (Math.abs(row.top - lastTop) > 30) {
                                uniqueRows.push(row);
                                lastTop = row.top;
                            }
                        }
                        return uniqueRows.map((r, idx) => ({ index: idx, top: r.top, height: r.height, text: r.text }));
                    }, item.vessel);
                    
                    console.log(`      Total de linhas encontradas: ${allRows.length}`);
                    if (allRows.length > 0) {
                        allRows.slice(0, 3).forEach((row, idx) => {
                            console.log(`        Linha ${idx}: top=${row.top}, h=${row.height}, "${row.text.substring(0, 50)}..."`);
                        });
                    }
                    
                    if (i >= allRows.length) {
                        console.log(`      ⚠️ Índice ${i} >= ${allRows.length} linhas totais`);
                        continue;
                    }
                    
                    // PASSO 4: Scroll até a linha desejada
                    const targetTop = allRows[i].top;
                    await page.evaluate((scrollTo) => {
                        window.scrollTo(0, Math.max(0, scrollTo - 300));
                    }, targetTop);
                    await new Promise(r => setTimeout(r, 800));
                    
                    // PASSO 5: Verificar novamente que não há painéis abertos
                    await page.evaluate(() => {
                    ...
                    // PASSO 6: Clicar na linha correta usando abordagem mais robusta
                    const clicked = await page.evaluate((targetIndex, expectedText, vesselName) => {
                        const rows = [];
                        const vesselParts = vesselName.toUpperCase().split(' ');
                        const vesselKey = vesselParts.length > 1 ? vesselParts.slice(0, 2).join(' ') : vesselParts[0];
                        
                        // Abordagem 1: Procurar botões/chevrons dentro de linhas de resultado
                        document.querySelectorAll('button, [role="button"], svg, [class*="chevron"], [class*="expand"], [class*="arrow"]').forEach(el => {
                            const parent = el.closest('div, article, section');
                            if (!parent) return;
                            ...
                            const goodSize = rect.width > 300 && rect.height > 30;
                            
                            if (hasDays && hasRouteType && isVisible && goodSize) {
                                rows.push({ 
                                    el: parent, 
                                    btn: el,
                                    top: rect.top, 
                        ...
                        // Abordagem 2 (fallback): Procurar divs diretamente
                        if (rows.length === 0) {
                            document.querySelectorAll('div, article, section').forEach(el => {
                                const text = el.innerText || '';
                                const rect = el.getBoundingClientRect();
                                ...
                                if (hasDays && hasRouteType && isVisible && goodSize) {
                                    rows.push({ 
                                        el: el, 
                                        btn: el.querySelector('button, svg, [class*="chevron"]'),
                                        top: rect.top, 
                                        text: text.substring(0, 100).replace(/\n/g, ' ')
                                    });
                                }
                        ...
                        // Encontrar a linha pelo texto esperado ou pelo navio
                        let targetRow = null;
                        
                        // Primeiro: tentar encontrar pelo texto exato
                        for (const row of uniqueRows) {
                            const expectedPart = expectedText.substring(0, 30);
                            if (row.text.includes(expectedPart)) {
                                targetRow = row;
                                break;
                            }
                        }
                        
                        // Segundo: tentar encontrar pelo nome do navio
                        if (!targetRow) {
                            for (const row of uniqueRows) {
                                if (row.text.toUpperCase().includes(vesselKey)) {
                                    targetRow = row;
                                    break;
                                }
                            }
                        ...
                        // Terceiro (fallback): pegar pela posição no índice
                        if (!targetRow && uniqueRows.length > 0) {
                            targetRow = uniqueRows[Math.min(targetIndex, uniqueRows.length - 1)];
                        }
                        
                        if (!targetRow) {
                            return { success: false, error: 'Nenhuma linha encontrada', rowsFound: rows.length, uniqueFound: uniqueRows.length };
                        }
                        
                        // Clicar no botão de expandir (preferir o botão específico se existir)
                        if (targetRow.btn) {
                            targetRow.btn.click();
                        } else {
                        ...
                        return { success: true, rowText: targetRow.text.substring(0, 60), rowTop: targetRow.top, rowsFound: rows.length };
                    }, i, allRows[i].text, item.vessel);
                    
                    if (clicked && clicked.success) {
                        console.log(`      Clicou na linha ${i+1}: "${clicked.rowText.substring(0,40)}..."`);
                        
                        // PASSO 7: Aguardar expansão do painel
                        await new Promise(r => setTimeout(r, 2500));
                        
                        // Screenshot para debug
                        await page.screenshot({ path: path.join(EXPORTS_DIR, `msc-09-expandido-${i+1}.png`) });
                        
                        // Extrair dados do painel expandido
                        const details = await page.evaluate((vesselName, currentService, polName, podName, expectedETA) => {
                            let service = '-';
                            let transbordo = '-';
                            let transbordoDate = '';
                            
                            // Buscar o painel expandido que contém o NAVIO correto
                            let panelText = '';
                            let vesselInPanel = false;
                            
                            // Extrair parte do nome do navio para busca (ex: "MSC MIA" -> "MIA")
                            const vesselParts = vesselName.toUpperCase().split(' ').filter(p => p.length > 2);
                            const vesselKey = vesselParts.length > 1 ? vesselParts[1] : vesselParts[0];
                            
                            // Procurar seções que contêm informações de rota
                            const panels = document.querySelectorAll('div, section, article');
                            for (const panel of panels) {
                                const text = panel.innerText || '';
                                const rect = panel.getBoundingClientRect();
                                
                                // Painel expandido deve conter informações de porto
                                if ((text.includes('Port of Discharge') || text.includes('Porto de Descarga') || 
                                     text.includes('Estimated Time') || text.includes('Hora estimada')) &&
                                    text.match(/\([A-Z]{5,6}\)/) &&
                                    rect.height > 150 && rect.height < 1000 &&
                                    rect.width > 600) {
                                    
                                    // Verificar se este painel contém o nome do navio
                                    const textUpper = text.toUpperCase();
                                    if (textUpper.includes(vesselKey) || textUpper.includes(vesselName.toUpperCase())) {
                                        panelText = text;
                                        vesselInPanel = true;
                                        break;
                                    }
                                    
                                    // Se não encontrou pelo navio, guardar como fallback
                                    if (!panelText) {
                                        panelText = text;
                                    }
                                }
                            ...
                            // Se não encontrou painel específico, usar body
                            if (!panelText) {
                            ...
                            for (let i = 0; i < lines.length; i++) {
                                ...
                                // Procurar código de porto - formato: "Nome (CODIGO)"
                                const portMatch = line.match(/^(.+?)\s*\(([A-Z]{5,6})\)$/);
                                if (portMatch) {
                                    const portName = portMatch[1].trim();
                                    const portCode = portMatch[2];
                                    const shortCode = portCode.substring(2);
                                    
                                    // Buscar serviço nas próximas linhas
                                    let portService = '-';
                                    for (let j = i + 1; j <= Math.min(lines.length - 1, i + 8); j++) {
                                        const serviceLine = lines[j];
                                        if (serviceLine.match(/\([A-Z]{5,6}\)$/)) break;
                                        
                                        for (const svc of mainServices) {
                                            if (serviceLine === svc + ' Service' || 
                                                serviceLine === svc ||
                                                serviceLine.includes(svc + ' Service')) {
                                                portService = svc;
                                                break;
                                            }
                                        }
                                        if (portService !== '-') break;
                                    }
                                    
                                    // Buscar data de chegada
                                    let portDate = '';
                                    for (let j = i + 1; j <= Math.min(lines.length - 1, i + 5); j++) {
                                        const dateLine = lines[j];
                                        if (dateLine.match(/\([A-Z]{5,6}\)$/)) break;
                                        
                                        const dateMatch = dateLine.match(/^(\w{3}\s+\d{1,2}(?:st|nd|rd|th)?\s+\w{3}\s+\d{4})/);
                                        if (dateMatch) {
                                            portDate = dateMatch[1];
                                            break;
                                        }
                                    }
                            ...
                            // Analisar portos encontrados
                            if (portsFound.length >= 2) {
                                // PRIMEIRO: Pegar serviço do POL (primeiro porto)
                                if (portsFound[0].service !== '-') {
                                    service = portsFound[0].service;
                                }
                                
                                // SEGUNDO: Se tem 3+ portos, há transbordo
                                if (portsFound.length >= 3) {
                                    // Pegar o primeiro porto intermediário como transbordo
                                    const transbordoPort = portsFound[1]; // Índice 1 = primeiro após POL
                                    transbordo = transbordoPort.code;
                                    transbordoDate = transbordoPort.date;
                                    
                                    // Se o transbordo tem serviço, usar ele (sobrescreve POL)
                                    if (transbordoPort.service !== '-') {
                                        service = transbordoPort.service;
                                    }
                                    
                                    // Procurar em todos os portos intermediários por serviço
                                    for (let p = 1; p < portsFound.length - 1; p++) {
                                        if (portsFound[p].service !== '-') {
                                            service = portsFound[p].service;
                                            // Se o serviço está neste porto, ele é o transbordo real
                                            transbordo = portsFound[p].code;
                                            transbordoDate = portsFound[p].date;
                                            break;
                                        }
                                    }
                                }
                                // Se tem só 2 portos = rota direta, sem transbordo
                            }
                            
                            // PRIORIDADE 4: Se ainda não tem serviço, procurar nas linhas do texto
                            if (service === '-') {
                                for (const line of lines) {
                                    for (const svc of mainServices) {
                                        if (line === svc + ' Service' || 
                                            line === svc ||
                                            line.includes(svc + ' Service')) {
                                            service = svc;
                                            break;
                                        }
                                    }
                            ...
                            // Se não encontrou serviço, manter o que já tinha
                            if (service === '-' && currentService && currentService !== '-') {
                            ...
                            return { 
                                ...
                                transbordo, 
                                transbordoDate, 
                                portsCount: portsFound.length, 
                                ports: portsFound.map(p => p.name + '(' + p.code + ')=' + p.service),
                                panelSize: panelText.length,
                                vesselFound: vesselInPanel,
                                polService: portsFound.length > 0 ? portsFound[0].service : 'N/A',
                                debug: `POL=${portsFound[0]?.service}, service final=${service}`
                            };
                        }, item.vessel, item.service, pol, pod, item.eta);
                        ...
                        console.log(`        Portos encontrados (${details.portsCount}): ${details.ports ? details.ports.join(' → ') : 'nenhum'}`);
                        console.log(`        DEBUG: ${details.debug}`);
                        
                        // IMPORTANTE: Fechar o painel clicando na mesma linha novamente
                        await page.evaluate(() => {
                            // Procurar e clicar em TODOS os painéis expandidos
                            document.querySelectorAll('[aria-expanded="true"], [class*="expanded"], [class*="open"]').forEach(el => {
                                const btn = el.querySelector('button, svg, [class*="chevron"]');
                                if (btn) btn.click();
                                else el.click();
                            ...
                            // Também procurar botões de fechar/minimizar
                            document.querySelectorAll('button, [role="button"]').forEach(el => {
                                const text = (el.innerText || '').toLowerCase();
                                const ariaLabel = (el.getAttribute('aria-label') || '').toLowerCase();
                                if (text.includes('close') || text.includes('collapse') || 
                                    ariaLabel.includes('close') || ariaLabel.includes('collapse')) {
                                    el.click();
                                }
                            });
                        });
                        
                        // Também pressionar ESC para garantir
                        await page.keyboard.press('Escape');
                        
                        // Esperar o painel fechar
                        await new Promise(r => setTimeout(r, 1500));
