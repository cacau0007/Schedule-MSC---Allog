                ...
                try {
                    ...
                    const allRows = await page.evaluate(() => {
                        ...
                        document.querySelectorAll('div, article, section').forEach(el => {
                            const text = el.innerText || '';
                            const rect = el.getBoundingClientRect();
                            // Linha de header tem altura entre 50-150px
                            if (text.includes('MSC') && text.includes('Days') &&
                                (text.includes('Transhipment') || text.includes('Direct')) &&
                    ...
                    if (clicked && clicked.success) {
                        ...
                        const details = await page.evaluate((vesselName, currentService, polName, podName, expectedETA) => {
                            ...
                            // Buscar o painel expandido que contém o ETA correto
                            // Isso garante que estamos lendo o painel certo
                            let panelText = '';
                            ...
                            for (const panel of panels) {
                                ...
                                // Painel expandido geralmente tem:
                                // - "Port of Discharge" ou "Porto de Descarga"
                                // - Altura maior que 200px
                                // - Contém nomes de porto com códigos
                                if ((text.includes('Port of Discharge') || text.includes('Porto de Descarga') || 
                                    ...
                                    // Verificar se este painel contém o ETA esperado
                                    if (expectedETA && text.includes(expectedETA.split('/')[0])) {
                                        panelText = text;
                                        break;
                                    }
                                    // Se não tem ETA esperado, usar o primeiro painel válido
                                    if (!panelText) {
                                        panelText = text;
                                    }
                                }
                            ...
                            // Se não encontrou painel específico, usar body (mas filtrado)
                            if (!panelText) {
                            ...
                            if (portsFound.length >= 2) {
                                // PRIORIDADE 1: Porto intermediário com serviço principal (transbordo)
                                ...
                                // PRIORIDADE 2: Se não achou serviço em transbordo, pegar penúltimo porto
                                if (transbordo === '-' && portsFound.length >= 3) {
                                    const penultimo = portsFound[portsFound.length - 2];
                                    transbordo = penultimo.code;
                                    transbordoDate = penultimo.date;
                                    if (penultimo.service !== '-') {
                                        service = penultimo.service;
                                    }
                                }
                                
                                // PRIORIDADE 3: Se ainda não tem serviço, procurar em QUALQUER porto (incluindo POL)
                                if (service === '-') {
                                    for (let p = 0; p < portsFound.length; p++) {
                                        if (portsFound[p].service !== '-') {
                                            service = portsFound[p].service;
                                            break;
                                        }
