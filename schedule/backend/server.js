...
// PODs que são atendidos via CONEXÃO/FEEDER - NÃO têm serviços diretos mapeados
// VAZIO - todos os PODs agora têm mapeamento específico no SERVICE_ROUTES
const CONNECTION_PODS = [];

// Função para verificar se é POL de conexão
function isConnectionRoute(pol) {
    return CONNECTION_POLS.includes(pol);
}

// Função para verificar se é POD de conexão
...
const SERVICE_ROUTES = {
    // ========================================================
    // SANTOS - Carioca, Ipanema, Santana, Jade (todos os 4)
    // ========================================================
    ...
    // ========================================================
    // RIO DE JANEIRO - Carioca, Santana, Jade (SEM Ipanema)
    // ========================================================
    ...
    // ========================================================
    // PARANAGUÁ - Carioca, Ipanema, Santana, Jade (todos os 4)
    // ========================================================
    ...
    // ========================================================
    // ITAPOÁ - Carioca, Jade (SEM Ipanema e Santana)
    // ========================================================
    ...
    // ========================================================
    // ITAGUAÍ - Carioca, Santana (SEM Jade e Ipanema)
    // ========================================================
    ...
    // ========================================================
    // IMBITUBA - Santana (SEM Jade, Carioca, Ipanema)
    // ========================================================
    ...
    // ========================================================
    // ITAJAÍ - Santana (SEM Jade, Carioca, Ipanema)
    // ========================================================
    ...
    // ========================================================
    // MONTEVIDEO - Ipanema (SEM Jade, Carioca, Santana)
    // ========================================================
    ...
    // ========================================================
    // BUENOS AIRES - Ipanema (SEM Jade, Carioca, Santana)
    // ========================================================
    ...
    // ========================================================
    // RIO GRANDE - Ipanema (SEM Jade, Carioca, Santana)
    // ========================================================
...
app.get('/api/available-services', (req, res) => {
    ...
    // Verificar se é POL de conexão
    if (isConnectionRoute(pol)) {
        return res.json({ 
            services: ALL_SERVICES,
            mapped: false,
            isConnection: true,
            message: `Rota de conexão (POL: ${pol}) - busca sem filtro`
        });
    }
    
    // Verificar se é POD de conexão (Manaus, Vitória, etc.)
    if (isConnectionPod(pod)) {
        return res.json({ 
            services: ALL_SERVICES,
            mapped: false,
            isConnection: true,
            message: `Rota de conexão (POD: ${pod}) - busca sem filtro`
        });
    }
...
async function scrapeMSC(pol, pod, service = null) {
    ...
    try {
        ...
        // 8. Para rotas de CONEXÃO: clicar em CADA resultado para obter serviço e transbordo
        let finalResults = data.results;
        
        if (isConnection && data.results.length > 0) {
            console.log('8. Extraindo detalhes de CADA navio (rota de conexão)...');
            ...
            for (let i = 0; i < Math.min(data.results.length, 20); i++) {
                ...
                console.log(`   [${i+1}/${Math.min(data.results.length, 20)}] ${item.vessel}...`);
                
                try {
                    // ESTRATÉGIA MELHORADA:
                    // 1. Encontrar o elemento que contém EXATAMENTE o nome do navio
                    // 2. Fazer scroll até ele
                    // 3. Clicar nele
                    
                    const clickResult = await page.evaluate(async (vesselName, etdText) => {
                        // Função para encontrar elemento por texto
                        ...
                        // Encontrar o elemento do navio
                        const vesselElement = findElementByText(vesselName);
                        if (!vesselElement) {
                            return { success: false, error: 'Navio não encontrado' };
                        }
                        
                        // Subir na árvore DOM até encontrar um elemento clicável (card/row)
                        let clickableElement = vesselElement;
                        for (let i = 0; i < 10; i++) {
                            if (!clickableElement.parentElement) break;
                            clickableElement = clickableElement.parentElement;
                            
                            const rect = clickableElement.getBoundingClientRect();
                            // Card de resultado típico: largura > 500px, altura entre 50-200px
                            if (rect.width > 500 && rect.height > 40 && rect.height < 250) {
                                break;
                            }
                        }
                        
                        // Fazer scroll até o elemento
                        clickableElement.scrollIntoView({ behavior: 'instant', block: 'center' });
                        
                        // Pequena pausa para scroll
                        await new Promise(r => setTimeout(r, 300));
                        ...
                        return { 
                            ...
                            elementClass: clickableElement.className
                        };
                    }, item.vessel, item.etd);
                    
                    if (clickResult.success) {
                        console.log(`      Clicou em ${clickResult.elementTag}`);
                        
                        // Esperar modal/expansão abrir
                        await new Promise(r => setTimeout(r, 2000));
                        
                        // Screenshot para debug
                        await page.screenshot({ path: path.join(EXPORTS_DIR, `msc-09-detalhe-${i+1}.png`) });
                        ...
                        const details = await page.evaluate(() => {
                            ...
                            const knownServices = ['Santana', 'Carioca', 'Ipanema', 'Jade'];
                            let service = '-';
                            let transbordo = '-';
                            let transbordoDate = '';
                            
                            // Procurar SERVIÇO - pode aparecer como "Ipanema Service", "Service: Santana", etc.
                            for (const line of lines) {
                                for (const svc of knownServices) {
                                    // Padrões comuns de serviço
                                    if (line === svc ||
                                        line === svc + ' Service' ||
                                        line.includes(svc + ' Service') ||
                                        line.match(new RegExp(`^${svc}$`, 'i')) ||
                                        line.match(new RegExp(`Service.*${svc}`, 'i')) ||
                                        line.match(new RegExp(`${svc}.*Service`, 'i'))) {
                                        service = svc;
                                        break;
                                    }
                                }
                            ...
                            // Procurar TRANSBORDO - porto intermediário com data
                            // Padrões: "Singapore", "SGSIN", "Busan", "KRPUS", etc.
                            for (let i = 0; i < lines.length; i++) {
                                const line = lines[i];
                                ...
                                if (lineLower.includes('singapore') || line.includes('SGSIN')) {
                                    foundPort = 'Singapore';
                                    portCode = 'SIN';
                                } else if (lineLower.includes('busan') || line.includes('KRPUS')) {
                                    foundPort = 'Busan';
                                    portCode = 'BUS';
                                } else if (lineLower.includes('tanjung pelepas') || line.includes('MYTPP')) {
                                    foundPort = 'Tanjung Pelepas';
                                    portCode = 'TPP';
                                } else if (lineLower.includes('port klang') || line.includes('MYPKG')) {
                                    foundPort = 'Port Klang';
                                    portCode = 'PKG';
                                }
                                
                                if (foundPort && transbordo === '-') {
                                    transbordo = portCode;
                                    
                                    // Procurar data nas linhas próximas (antes e depois)
                                    for (let j = Math.max(0, i - 5); j <= Math.min(lines.length - 1, i + 5); j++) {
                                        const dateMatch = lines[j].match(/(\d{1,2}(?:st|nd|rd|th)?\s+(?:Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)\s+\d{4})/i);
                                        if (dateMatch && !transbordoDate) {
                                            // Verificar se não é a data de ETD ou ETA já conhecida
                        ...
                        console.log(`      ✅ Serviço: ${details.service}, Transbordo: ${details.transbordo} ${details.transbordoDate || ''}`);
                        ...
                        console.log(`      ⚠️ Falhou: ${clickResult.error}`);
                    }
...
// ============================================
// EXCEL - Geração simplificada e segura
// ============================================
async function generateExcel(sailings, pol, pod, filename) {
    try {
        ...
        sheet.columns = [
            ...
            { header: 'TRANSBORDO', key: 'transbordo', width: 22 },
