    ...
    try {
        ...
        if (isConnection && data.results.length > 0) {
            ...
            for (let i = 0; i < Math.min(data.results.length, 30); i++) {
                ...
                try {
                    // Clicar na linha do navio para expandir
                    const clicked = await page.evaluate((vesselName, etd) => {
                        ...
                        while (node = walker.nextNode()) {
                            if (node.textContent.includes(vesselName)) {
                                ...
                                while (el && el.tagName !== 'BODY') {
                                    const rect = el.getBoundingClientRect();
                                    if (rect.width > 500 && rect.height > 40 && rect.height < 150) {
                                        el.click();
                                        return true;
                                    }
                                    el = el.parentElement;
                    ...
                    if (clicked) {
                        ...
                        const details = await page.evaluate(() => {
                            ...
                            for (let i = 0; i < lines.length; i++) {
                                ...
                                if (line.includes('Singapore') && (line.includes('SG') || line.includes('SGSIN'))) {
                                    ...
                                    transbordo = 'Port Klang';
                                }
                                
                                // Se encontrou transbordo, pegar data próxima
                                if (transbordo !== '-' && !transbordoDate) {
                                    const dateMatch = lines.slice(Math.max(0, i-2), i+3).join(' ')
                                        .match(/(\d{1,2}(?:st|nd|rd|th)?\s+(?:Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)\s+\d{4})/i);
                        ...
                        console.log(`      Serviço: ${details.service}, Transbordo: ${details.transbordo}`);
