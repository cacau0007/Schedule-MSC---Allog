                ...
                try {
                    ...
                    if (clicked && clicked.success) {
                        ...
                        const details = await page.evaluate((vesselName, currentService, polName, podName) => {
                            ...
                            // Estrutura: POL -> Transbordo -> POD
                            // Cada porto aparece com código entre parênteses: "Valencia (ESVLC)" ou "Busan (KRPUS)"
                            ...
                            for (let i = 0; i < lines.length; i++) {
                                ...
                                // Procurar código de porto entre parênteses - formato mais flexível
                                // Aceita: "Nome (CODIGO)" onde CODIGO tem 5 caracteres alfanuméricos
                                const portMatches = line.match(/([A-Za-z][A-Za-z\s\-\.]+)\s*\(([A-Z]{2}[A-Z0-9]{2,3})\)/g);
                                if (portMatches) {
                                    for (const match of portMatches) {
                                        const parts = match.match(/([A-Za-z][A-Za-z\s\-\.]+)\s*\(([A-Z]{2}[A-Z0-9]{2,3})\)/);
                                        if (parts) {
                            ...
                            // Primeiro = POL, Último = POD, Meio = Transbordo
                            if (portsFound.length >= 3) {
                                // Tem transbordo (pelo menos 3 portos)
                                const transbordoPort = portsFound[1]; // Segundo porto é o transbordo
