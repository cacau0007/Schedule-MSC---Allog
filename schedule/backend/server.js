                    ...
                    if (clicked && clicked.success) {
                        ...
                        const details = await page.evaluate((vesselName, currentService, polName, podName) => {
                            ...
                            for (let i = 0; i < lines.length; i++) {
                                ...
                                // Procurar código de porto - formato: "Nome (CODIGO)" ou "Nome, Local (CODIGO)"
                                // Código tem 5-6 letras maiúsculas (ex: IDJKT, SGSIN, BRNVT, KRPUS)
                                const portMatch = line.match(/^(.+?)\s*\(([A-Z]{5,6})\)$/);
                                if (portMatch) {
                                    const portName = portMatch[1].trim();
                                    const portCode = portMatch[2];
                                    
                                    // Extrair últimas 3 letras do código
                                    const shortCode = portCode.substring(2);
                                    
                                    // Buscar serviço nas próximas linhas (até encontrar outro porto ou limite)
                                    let portService = '-';
                                    for (let j = i + 1; j <= Math.min(lines.length - 1, i + 8); j++) {
                                        const serviceLine = lines[j];
                                        
                                        // Parar se encontrar outro porto
                                        if (serviceLine.match(/\([A-Z]{5,6}\)$/)) break;
                                        
                                        // Verificar se é um serviço principal
                                    ...
                                    // Buscar data de chegada nas próximas linhas
                                    let portDate = '';
                            ...
                            if (portsFound.length >= 2) {
                                // PRIORIDADE 1: Procurar porto (não POL, não POD) com serviço principal
                                ...
                                // PRIORIDADE 2: Se não achou serviço principal, pegar penúltimo porto
                            ...
                            return { service, transbordo, transbordoDate, portsCount: portsFound.length, ports: portsFound.map(p => p.name + '(' + p.code + ')=' + p.service) };
