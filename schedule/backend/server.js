...
async function scrapeMSC(pol, pod, service = null) {
    ...
    try {
        ...
        if (isConnection && data.results.length > 0) {
            ...
            // Tirar screenshot antes de começar
            ...
            for (let i = 0; i < Math.min(data.results.length, 15); i++) {
                ...
                try {
                    // Método 1: Clicar diretamente na linha pelo índice
                    // Os resultados estão em uma lista/tabela, clicar no item i+1 (pular header)
                    const clicked = await page.evaluate((index, vesselName) => {
                        // Tentar encontrar todos os cards/linhas de resultado
                        // Procurar por elementos que contenham datas no formato do MSC
                        const allRows = [];
                        
                        // Método A: Procurar divs que parecem cards de resultado
                        document.querySelectorAll('div, tr, li, article').forEach(el => {
                            const text = el.innerText || '';
                            const rect = el.getBoundingClientRect();
                            
                            // Card de resultado: contém data, nome do navio, largura grande
                            if (text.includes('MSC') && 
                                text.match(/\d{1,2}(?:st|nd|rd|th)?\s+(?:Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)/i) &&
                                rect.width > 500 && 
                                rect.height > 40 && 
                                rect.height < 250 &&
                                rect.top > 0 &&
                                ...
                                // Verificar se não é um container maior
                                const parent = el.parentElement;
                                if (parent) {
                                    const parentRect = parent.getBoundingClientRect();
                                    // Se o parent tem altura muito maior, provavelmente este é o card certo
                                    if (parentRect.height > rect.height * 2) {
                                        allRows.push({ el, top: rect.top, text: text.substring(0, 100) });
                                    }
                                }
                            }
                        });
                        ...
                        for (const row of allRows) {
                            if (row.top - lastTop > 30) {
                                uniqueRows.push(row);
                                lastTop = row.top;
                            }
                        }
                        
                        // Encontrar o card que contém o nome do navio específico
                        let targetRow = null;
                        for (const row of uniqueRows) {
                            if (row.text.includes(vesselName)) {
                                targetRow = row;
                                break;
                            }
                        }
                        
                        // Se não encontrou pelo nome, usar índice
                        if (!targetRow && uniqueRows.length > index) {
                            targetRow = uniqueRows[index];
                        }
                        
                        if (!targetRow) {
                            return { success: false, error: `Nenhum card encontrado (total: ${uniqueRows.length})` };
                        }
                        ...
                        return new Promise(resolve => {
                            setTimeout(() => {
                                targetRow.el.click();
                                resolve({ success: true, cardText: targetRow.text.substring(0, 50) });
                            }, 200);
                        });
                    }, i, item.vessel);
                    ...
                    if (clicked && clicked.success) {
                        ...
                        // Screenshot do estado expandido
                        if (i === 0) {
                        ...
                        const details = await page.evaluate(() => {
                            ...
                            for (let i = 0; i < lines.length; i++) {
                                ...
                                // Procurar Singapore/Busan na lista expandida
                                ...
                                // Procurar data perto do transbordo
                        ...
                        // Fechar - clicar no mesmo card novamente ou ESC
                        await page.keyboard.press('Escape');
                        await new Promise(r => setTimeout(r, 500));
                        
                    } else {
                        console.log(`      ⚠️ ${clicked ? clicked.error : 'Erro desconhecido'}`);
                    }
