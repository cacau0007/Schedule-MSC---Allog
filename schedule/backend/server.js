...
async function scrapeMSC(pol, pod, service = null) {
    ...
    try {
        ...
        if (isConnection && basicData.length > 0) {
            ...
            for (let i = 0; i < Math.min(basicData.length, 10); i++) { // Limitar a 10 resultados
                const item = basicData[i];
                console.log(`   [${i+1}/${Math.min(basicData.length, 10)}] ${item.vessel}...`);
                
                try {
                    // Estratégia: encontrar a linha que contém o navio e clicar no ícone de expansão (seta/chevron)
                    const clicked = await page.evaluate((vesselName, index) => {
                        // Procurar por elementos que parecem ser linhas de resultado
                        const rows = document.querySelectorAll('[class*="result"], [class*="row"], [class*="card"], [class*="item"], tr, [role="row"]');
                        
                        for (const row of rows) {
                            const text = row.innerText || '';
                            if (text.includes(vesselName)) {
                                // Procurar por botão de expansão dentro da linha
                                const expandBtn = row.querySelector('[class*="expand"], [class*="chevron"], [class*="arrow"], [class*="toggle"], button, [role="button"]');
                                if (expandBtn) {
                                    expandBtn.click();
                                    return { clicked: true, method: 'expand-btn' };
                                }
                                
                                // Se não achar botão, clicar na própria linha
                                row.click();
                                return { clicked: true, method: 'row-click' };
                            }
                        }
                        
                        // Fallback: procurar qualquer elemento com o nome do navio
                        const allElements = Array.from(document.querySelectorAll('*'));
                        for (const el of allElements) {
                            if (el.innerText === vesselName || el.textContent === vesselName) {
                                const parent = el.closest('[class*="result"], [class*="row"], [class*="card"]') || el.parentElement;
                                if (parent) {
                                    parent.click();
                    ...
                    if (clicked.clicked) {
                        ...
                        await new Promise(r => setTimeout(r, 2000)); // Esperar expansão
                        ...
                        const details = await page.evaluate((vesselName) => {
                            ...
                            // Serviços conhecidos (incluindo os que aparecem como "Xxx Service")
                            ...
                            for (let i = 0; i < lines.length; i++) {
                                ...
                                // Marcar quando encontramos a área do navio que queremos
                                if (line.includes(vesselName)) {
                                    foundVessel = true;
                                }
                                
                                // Detectar serviço (procurar "Xxx Service" ou apenas o nome)
                                ...
                                // Detectar apenas o nome do serviço (sem "Service")
                                if (service === '-') {
                                    for (const svc of knownServices) {
                                        if (line === svc || line === `${svc} Service`) {
                                            service = svc;
                                            break;
                                        }
                                    }
                                ...
                                if (line.includes('Singapore') && (line.includes('SG') || line.includes('SGSIN'))) {
                                    transhipmentPort = 'Singapore';
                                    // Tentar pegar a data da linha anterior ou próxima
                                    const prevLine = lines[i-1] || '';
                                    const nextLine = lines[i+1] || '';
                                    const dateMatch = (prevLine + nextLine).match(/\d{1,2}(?:st|nd|rd|th)?\s+\w{3}\s+\d{4}/);
                                    if (dateMatch) transhipmentDate = dateMatch[0];
                                } else if (line.includes('Busan') && (line.includes('KR') || line.includes('KRPUS'))) {
                                    transhipmentPort = 'Busan';
                                } else if (line.includes('Tanjung Pelepas') || line.includes('MYTPP')) {
                                    transhipmentPort = 'Tanjung Pelepas';
                                } else if (line.includes('Port Klang') || line.includes('MYPKG')) {
...
async function generateExcel(sailings, pol, pod, filename) {
    ...
    try {
        ...
        console.error(`❌ Erro ao gerar Excel: ${excelError.message}`);
        // Tentar gerar um Excel mais simples
        const simpleWorkbook = new ExcelJS.Workbook();
        const simpleSheet = simpleWorkbook.addWorksheet('SCHEDULES');
        simpleSheet.addRow(['CARRIER', 'SERVIÇO', 'NAVIO', 'ETD', 'ETA', 'TRANSIT', 'TIPO', 'TRANSBORDO']);
        sailings.forEach(s => {
            simpleSheet.addRow([
                s.carrier || '-',
                s.service || '-',
        ...
        await simpleWorkbook.xlsx.writeFile(filepath);
        console.log(`📊 Excel simples gerado: ${filename}`);
    }
}
