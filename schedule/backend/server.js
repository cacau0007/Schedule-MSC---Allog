...
async function scrapeMSC(pol, pod, service = null) {
    ...
    try {
        ...
        if (service && service !== 'ALL') {
            ...
            try {
                // Clicar no botão "Filter by: All Services"
                const filterClicked = await page.evaluate(() => {
                    // Buscar especificamente pelo texto "Filter by"
                    const allElements = document.querySelectorAll('*');
                    for (const el of allElements) {
                        const text = (el.innerText || el.textContent || '').trim();
                        // Match exato ou próximo
                        if (text === 'Filter by: All Services' || 
                            text === 'Filter by:All Services' ||
                ...
                if (filterClicked.clicked) {
                    console.log(`   ✅ Dropdown clicado: "${filterClicked.text}" (y=${filterClicked.y})`);
                    ...
                    const options = await page.evaluate(() => {
                        const items = [];
                        const elements = document.querySelectorAll('li, [role="option"], [role="menuitem"], [role="listbox"] *, ul li, div[class*="option"], div[class*="menu"] div');
                        elements.forEach(el => {
                            const text = (el.innerText || el.textContent || '').trim();
                            const rect = el.getBoundingClientRect();
                            // Itens do dropdown: visíveis, tamanho razoável, texto de serviço
                            if (rect.height > 15 && rect.height < 50 && 
                                rect.width > 80 && 
                                text.length > 2 && text.length < 50 &&
                                !text.includes('@') && !text.includes('+') &&
                                !text.includes('http') && !text.includes('Search')) {
                                items.push({ text: text, y: Math.round(rect.y), h: Math.round(rect.height) });
                            }
                        });
                        // Remover duplicatas e ordenar
                        const unique = [...new Map(items.map(i => [i.text, i])).values()];
                        return unique.sort((a, b) => a.y - b.y).slice(0, 15);
                    });
                    ...
                    const serviceSelected = await page.evaluate((targetService) => {
                        const target = targetService.toLowerCase();
                        const elements = document.querySelectorAll('li, [role="option"], [role="menuitem"], div, span, a');
                        ...
                        for (const el of elements) {
                            ...
                            // Match flexível
                            if (rect.height > 15 && rect.height < 50 && rect.width > 80) {
                                if (textLower === target || 
                                    textLower === target + ' service' ||
                    ...
                    if (serviceSelected.selected) {
                        console.log(`   ✅ Serviço selecionado: "${serviceSelected.text}"`);
                        await new Promise(r => setTimeout(r, 2000));
                    } else {
                        console.log(`   ⚠️ Serviço "${service}" não encontrado`);
                    }
                } else {
                    console.log('   ⚠️ Botão "Filter by: All Services" não encontrado');
                    
                    // DEBUG: listar todos elementos com "Filter" ou "Service"
                    const debugElements = await page.evaluate(() => {
                        const results = [];
                        document.querySelectorAll('*').forEach(el => {
                            const text = (el.innerText || '').trim();
                            if ((text.includes('Filter') || text.includes('Service')) && text.length < 100) {
                                const rect = el.getBoundingClientRect();
                                if (rect.width > 50 && rect.height > 10) {
                                    results.push({ text: text.substring(0, 50), y: Math.round(rect.y), w: Math.round(rect.width), h: Math.round(rect.height), tag: el.tagName });
                                }
                            }
                        });
                        return results.slice(0, 10);
                    });
                    console.log('   DEBUG - Elementos com Filter/Service:');
                    debugElements.forEach(el => console.log(`      "${el.text}" y=${el.y} ${el.tag}`));
                }
