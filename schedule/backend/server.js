    ...
    try {
        ...
        if (isConnection && data.results.length > 0) {
            ...
            for (let i = 0; i < Math.min(data.results.length, 20); i++) {
                ...
                console.log(`   [${i+1}/${Math.min(data.results.length, 20)}] Clicando em ${item.vessel}...`);
                
                try {
                    // ESTRATÉGIA: Encontrar o card/linha que contém o navio e clicar
                    // O site MSC usa cards clicáveis que expandem para mostrar detalhes
                    const clicked = await page.evaluate((vesselName, itemIndex) => {
                        // Procurar todos os elementos que contêm o nome do navio
                        const allElements = document.querySelectorAll('*');
                        const candidates = [];
                        
                        for (const el of allElements) {
                            const text = el.innerText || '';
                            if (text.includes(vesselName) && !text.includes('DEPARTURE') && !text.includes('ARRIVAL')) {
                                const rect = el.getBoundingClientRect();
                                // Cards de resultado geralmente têm entre 60-200px de altura
                                if (rect.width > 400 && rect.height > 50 && rect.height < 250 && rect.top > 0) {
                                    candidates.push({ el, rect, text: text.substring(0, 100) });
                                }
                            }
                        }
                        
                        // Ordenar por posição vertical (top) e pegar o correto baseado no index
                        candidates.sort((a, b) => a.rect.top - b.rect.top);
                        
                        if (candidates.length > itemIndex) {
                            candidates[itemIndex].el.click();
                            return { clicked: true, info: `Card ${itemIndex + 1} de ${candidates.length}` };
                        } else if (candidates.length > 0) {
                            candidates[0].el.click();
                            return { clicked: true, info: `Primeiro card (total: ${candidates.length})` };
                        }
                        
                        return { clicked: false, info: 'Nenhum card encontrado' };
                    }, item.vessel, i);
                    
                    if (clicked.clicked) {
                        console.log(`      Clicou: ${clicked.info}`);
                        await new Promise(r => setTimeout(r, 2500)); // Esperar modal/expansão abrir
                        
                        // Screenshot do modal/detalhes
                        await page.screenshot({ path: path.join(EXPORTS_DIR, `msc-09-detalhe-${i+1}.png`) });
                        
                        // Extrair serviço e transbordo do conteúdo expandido
                        const details = await page.evaluate(() => {
                            ...
                            // Procurar por padrões específicos de serviço
                            // Ex: "Ipanema Service", "Santana", "Via Jade"
                            for (let i = 0; i < lines.length; i++) {
                                const line = lines[i];
                                const lineLower = line.toLowerCase();
                                
                                // Detectar serviço - várias formas que pode aparecer
                                for (const svc of knownServices) {
                                    if (line.includes(svc + ' Service') || 
                                        line === svc || 
                                        line.includes('Service: ' + svc) ||
                                        line.includes('Via ' + svc) ||
                                        (lineLower.includes('service') && line.includes(svc))) {
                                        service = svc;
                                        break;
                                    }
                                }
                                ...
                                // Procurar por linhas que parecem ser paradas intermediárias
                                if (line.includes('Singapore') || lineLower.includes('sgsin')) {
                                    transbordo = 'SIN';
                                    // Procurar data próxima
                                ...
                                } else if (line.includes('Busan') || lineLower.includes('krpus')) {
                                    transbordo = 'BUS';
                                    for (let j = Math.max(0, i-3); j <= Math.min(lines.length-1, i+3); j++) {
                                        const dateMatch = lines[j].match(/(\d{1,2}(?:st|nd|rd|th)?\s+(?:Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)\s+\d{4})/i);
                                        if (dateMatch) {
                                ...
                                } else if (line.includes('Tanjung Pelepas') || lineLower.includes('mytpp')) {
                                    transbordo = 'TPP';
                                    for (let j = Math.max(0, i-3); j <= Math.min(lines.length-1, i+3); j++) {
                                        const dateMatch = lines[j].match(/(\d{1,2}(?:st|nd|rd|th)?\s+(?:Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)\s+\d{4})/i);
                                        if (dateMatch) {
                                ...
                                } else if (line.includes('Port Klang') || lineLower.includes('mypkg')) {
                                    transbordo = 'PKG';
                                    for (let j = Math.max(0, i-3); j <= Math.min(lines.length-1, i+3); j++) {
                                        const dateMatch = lines[j].match(/(\d{1,2}(?:st|nd|rd|th)?\s+(?:Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)\s+\d{4})/i);
                                        if (dateMatch) {
                        ...
                        // Atualizar o item com os detalhes extraídos
                        ...
                        // Fechar modal/voltar - tentar várias formas
                        try {
                            // Tentar ESC
                            await page.keyboard.press('Escape');
                            await new Promise(r => setTimeout(r, 800));
                            
                            // Verificar se fechou, se não, clicar fora
                        ...
                        console.log(`      ⚠️ Não encontrou card: ${clicked.info}`);
                    }
                } catch (e) {
                    console.log(`      ❌ Erro: ${e.message}`);
                }
                
                // Pequena pausa entre cada clique
                await new Promise(r => setTimeout(r, 300));
            }
        }
