    ...
    try {
        ...
        if (isConnection && data.results.length > 0) {
            ...
            for (let i = 0; i < Math.min(data.results.length, 20); i++) {
                ...
                try {
                    // Clicar no card do navio para expandir
                    const clicked = await page.evaluate((vesselName) => {
                        // Encontrar elemento com nome do navio
                        const allElements = document.querySelectorAll('*');
                        for (const el of allElements) {
                            if (el.innerText && el.innerText.includes(vesselName) && !el.innerText.includes('Porto')) {
                                // Subir até encontrar o card clicável (linha do resultado)
                                let card = el;
                                for (let i = 0; i < 15; i++) {
                                    if (!card.parentElement) break;
                                    card = card.parentElement;
                                    const rect = card.getBoundingClientRect();
                                    // Card típico: largura grande, altura média
                                    if (rect.width > 600 && rect.height > 50 && rect.height < 200) {
                    ...
                    if (clicked) {
                        ...
                        // Estrutura: tabela com colunas Porto | Chegada | Partida | Serviço | Navio
                        const details = await page.evaluate((vesselName) => {
                            ...
                            // Procurar por Singapore ou Busan na lista de portos
                            // e pegar o serviço associado
                            for (let i = 0; i < lines.length; i++) {
                                const line = lines[i];
                                const lineLower = line.toLowerCase();
                                
                                // Detectar linha de transbordo (Singapore, Busan, etc)
                                if (lineLower.includes('singapore') || line.includes('SGSIN')) {
                                    transbordo = 'SIN';
                                    
                                    // Procurar serviço nas linhas próximas (mesma linha ou adjacentes)
                                    // Serviços conhecidos: Ipanema, Santana, Carioca, Jade + feeders
                                    for (let j = Math.max(0, i - 3); j <= Math.min(lines.length - 1, i + 3); j++) {
                                        const nearLine = lines[j];
                                        if (nearLine.includes('Ipanema')) {
                                            service = 'Ipanema';
                                        } else if (nearLine.includes('Santana')) {
                                            service = 'Santana';
                                        } else if (nearLine.includes('Carioca')) {
                                            service = 'Carioca';
                                        } else if (nearLine.includes('Jade')) {
                                            service = 'Jade';
                                        }
                                        
                                        // Pegar data (formato: "Tue 27th Jan 2026" ou similar)
                                        const dateMatch = nearLine.match(/(\w{3}\s+\d{1,2}(?:st|nd|rd|th)?\s+\w{3}\s+\d{4})/i);
                                    ...
                                    for (let j = Math.max(0, i - 3); j <= Math.min(lines.length - 1, i + 3); j++) {
                                        const nearLine = lines[j];
                                        if (nearLine.includes('Ipanema')) service = 'Ipanema';
                                        else if (nearLine.includes('Santana')) service = 'Santana';
                                        else if (nearLine.includes('Carioca')) service = 'Carioca';
                                        else if (nearLine.includes('Jade')) service = 'Jade';
                                        
                                        const dateMatch = nearLine.match(/(\w{3}\s+\d{1,2}(?:st|nd|rd|th)?\s+\w{3}\s+\d{4})/i);
                                        if (dateMatch && !transbordoDate) transbordoDate = dateMatch[1];
                                    }
                                ...
                                // Se ainda não achou serviço, procurar "Service" na linha
                                if (service === '-' && line.includes('Service')) {
                                    if (line.includes('Ipanema')) service = 'Ipanema';
                                    else if (line.includes('Santana')) service = 'Santana';
                                    else if (line.includes('Carioca')) service = 'Carioca';
                                    else if (line.includes('Jade')) service = 'Jade';
                                }
                            }
                        ...
                        // Fechar painel (clicar novamente ou ESC)
