                    ...
                    if (clicked && clicked.success) {
                        ...
                        const details = await page.evaluate((vesselName, currentService, polName, podName) => {
                            ...
                            for (let i = 0; i < lines.length; i++) {
                                ...
                                // Procurar código de porto entre parênteses no INÍCIO da linha
                                // Formato: "Nome (CODIGO)" ou "Nome, Local (CODIGO)"
                                ...
                                if (portMatch) {
                                    ...
                                    // Verificar se tem serviço principal associado (nas próximas 5 linhas)
                                    let portService = '-';
                                    for (let j = i + 1; j <= Math.min(lines.length - 1, i + 6); j++) {
                                        const serviceLine = lines[j];
                                        for (const svc of mainServices) {
                                            // Verificar "Santana Service" ou só "Santana" como linha separada
                                            if (serviceLine === svc + ' Service' || 
                                                serviceLine === svc ||
                                                serviceLine.startsWith(svc + ' Service')) {
                                                portService = svc;
                                                break;
                                            }
                                        }
                                        ...
                                        if (lines[j].match(/\([A-Z]{5,6}\)$/)) break;
                                    }
                                    
                                    // Buscar data de chegada (primeira data após o porto)
                                    let portDate = '';
                            ...
                            if (portsFound.length >= 3) {
                                ...
                                // PRIORIDADE 1: Porto com serviço Santana/Carioca/Ipanema/Jade
                                // Percorrer do fim para o início (excluindo POL e POD)
                                for (let p = portsFound.length - 2; p >= 1; p--) {
                                ...
                                // PRIORIDADE 2: Se não achou serviço principal, pegar penúltimo
                                if (!transbordoPort) {
