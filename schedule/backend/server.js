    ...
    try {
        ...
        if (isConnection && data.results.length > 0) {
            ...
            for (let i = 0; i < Math.min(data.results.length, 30); i++) {
                ...
                console.log(`   [${i+1}/${Math.min(data.results.length, 30)}] ${item.vessel}...`);
                
                try {
                    // Clicar na linha do navio e extrair dados do elemento expandido
                    const details = await page.evaluate(async (vesselName, vesselEtd, index) => {
                        ...
                        // Encontrar o card clicável para este navio específico
                        // Usar o índice para evitar clicar sempre no mesmo
                        let targetCard = null;
                        const cards = [];
                        ...
                        for (const textNode of allNodes) {
                            ...
                            for (let j = 0; j < 12; j++) {
                                ...
                                const rect = el.getBoundingClientRect();
                                if (rect.width > 500 && rect.height > 40 && rect.height < 200 && rect.top > 0) {
                                    cards.push({ el, top: rect.top });
                                    break;
                                }
                                el = el.parentElement;
                            }
                        ...
                        // Ordenar por posição e pegar pelo índice
                        ...
                        targetCard = uniqueCards[Math.min(index, uniqueCards.length - 1)];
                        if (!targetCard) {
                            return { success: false, error: 'Index inválido' };
                        }
                        
                        // Scroll e clique
                        targetCard.el.scrollIntoView({ block: 'center' });
                        await new Promise(r => setTimeout(r, 300));
                        targetCard.el.click();
                        ...
                        // Agora extrair do elemento expandido ou modal
                        // Procurar por elementos que apareceram após o clique
                        let textSource = targetCard.el;
                        
                        // Tentar encontrar modal ou painel expandido
                        const possibleModals = document.querySelectorAll('[class*="modal"], [class*="detail"], [class*="expand"], [class*="panel"]');
                        for (const modal of possibleModals) {
                            if (modal.offsetParent !== null && modal.innerText.length > 100) {
                                textSource = modal;
                                break;
                            }
                        }
                        ...
                        for (let i = 0; i < lines.length; i++) {
                            ...
                            // Detectar serviço
                            for (const svc of knownServices) {
                                if (line.includes(svc + ' Service') || line === svc || lineLower.includes(svc.toLowerCase() + ' service')) {
                                    service = svc;
                                    break;
                                }
                            }
                            ...
                            // Detectar transbordo
                            if (transbordo === '-') {
                                if ((lineLower.includes('singapore') || line.includes('SGSIN')) && !lineLower.includes('from singapore')) {
                                    transbordo = 'SIN';
                            ...
                            // Se encontrou transbordo, pegar data
