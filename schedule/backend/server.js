        ...
        if (service) {
            ...
            try {
                ...
                if (filterPos.found) {
                    ...
                    const servicePos = await page.evaluate((targetText) => {
                        const allElements = document.querySelectorAll('*');
                        let bestMatch = null;
                        ...
                        for (const el of allElements) {
                            ...
                            if (text === targetText && rect.y > 0 && rect.width > 0 && rect.height > 0) {
                    ...
                    if (servicePos.found) {
                        console.log(`   ✅ ${serviceText} em (${Math.round(servicePos.x)}, ${Math.round(servicePos.y)})`);
