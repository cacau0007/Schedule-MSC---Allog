...
async function scrapeMSC(pol, pod, service = null) {
    ...
    try {
        ...
        if (isConnection && data.results.length > 0) {
            console.log('8. Extraindo detalhes (rota de conexão)...');
            ...
            for (let i = 0; i < Math.min(data.results.length, 15); i++) {
                ...
                console.log(`   [${i+1}/${Math.min(data.results.length, 15)}] ${item.vessel}...`);
                
                try {
                    // O site MSC tem um botão de expandir (seta v) no final de cada linha
                    // Ou podemos clicar no nome do navio
                    // Vamos procurar todos os botões de expandir e clicar no índice correto
                    
                    const clicked = await page.evaluate(async (index, vesselName) => {
                        // Método 1: Procurar botões de expandir (svg ou botão com seta)
                        const expandButtons = document.querySelectorAll('button svg, [class*="expand"], [class*="chevron"], [class*="arrow"], button[aria-expanded]');
                        
                        // Método 2: Procurar linhas de resultado e o botão dentro delas
                        const rows = [];
                        document.querySelectorAll('div, tr, article').forEach(el => {
                            const text = el.innerText || '';
                            const rect = el.getBoundingClientRect();
                            
                            // Linha de resultado: contém nome do navio, datas, "Days", "Transhipment"
                        ...
                        if (uniqueRows.length === 0) {
                            return { success: false, error: 'Nenhuma linha encontrada' };
                        }
                        
                        // Encontrar a linha correta pelo nome do navio ou índice
                        let targetRow = uniqueRows.find(r => r.text.includes(vesselName));
                        if (!targetRow && index < uniqueRows.length) {
                            targetRow = uniqueRows[index];
                        }
                        
                        if (!targetRow) {
                            return { success: false, error: `Linha não encontrada (${uniqueRows.length} linhas)` };
                        }
                        ...
                        // Procurar o botão de expandir DENTRO da linha
                        const expandBtn = targetRow.el.querySelector('button, [role="button"], svg');
                        if (expandBtn) {
                            expandBtn.click();
                            return { success: true, method: 'botão expandir' };
                        }
                        
                        // Se não achou botão, clicar na linha inteira
                        targetRow.el.click();
                        return { success: true, method: 'linha inteira' };
                        
                    }, i, item.vessel);
                    
                    if (clicked && clicked.success) {
                        console.log(`      Clicou (${clicked.method})`);
                        
                        // Esperar expansão
                        await new Promise(r => setTimeout(r, 2500));
                        
                        // Screenshot do primeiro expandido
                        if (i === 0) {
                        ...
                        const details = await page.evaluate(() => {
                            ...
                            // Procurar Singapore/Busan (porto de transbordo)
                            // e o serviço associado (Ipanema Service, Santana Service, etc)
                            for (let i = 0; i < lines.length; i++) {
                                const line = lines[i];
                                const lower = line.toLowerCase();
                                
                                // Porto de transbordo
                                if ((lower.includes('singapore') || line.includes('SGSIN')) && transbordo === '-') {
                                    transbordo = 'SIN';
                                }
                                if ((lower.includes('busan') || line.includes('KRPUS')) && transbordo === '-') {
                                    transbordo = 'BUS';
                                }
                                if ((lower.includes('tanjung pelepas') || line.includes('MYTPP')) && transbordo === '-') {
                                    transbordo = 'TPP';
                                }
                                
                                // Serviço (procurar "Ipanema Service", "Santana", etc)
                                if (line.includes('Ipanema') && service === '-') service = 'Ipanema';
                                if (line.includes('Santana') && service === '-') service = 'Santana';
                                if (line.includes('Carioca') && service === '-') service = 'Carioca';
                                if (line.includes('Jade') && service === '-') service = 'Jade';
                                
                                // Data do transbordo
                                if (transbordo !== '-' && !transbordoDate) {
                                    const dateMatch = line.match(/(\w{3}\s+\d{1,2}(?:st|nd|rd|th)?\s+\w{3}\s+\d{4})/);
                                    if (dateMatch) transbordoDate = dateMatch[1];
                                }
                            }
                        ...
                        // Fechar - clicar novamente ou ESC
...
async function generateExcel(sailings, pol, pod, filename) {
    ...
    try {
        // Usar writeBuffer para garantir arquivo válido
        const buffer = await workbook.xlsx.writeBuffer();
        const fs = require('fs');
        fs.writeFileSync(filepath, buffer);
        console.log(`📊 Excel gerado: ${filename} (${buffer.length} bytes)`);
    } catch (err) {
        console.error(`❌ Erro ao gerar Excel: ${err.message}`);
        // Fallback: tentar writeFile normal
        await workbook.xlsx.writeFile(filepath);
        console.log(`📊 Excel gerado (fallback): ${filename}`);
    }
}
