...
async function scrapeMSC(pol, pod, service = null) {
    ...
    try {
        ...
        if (service && service !== 'ALL') {
            ...
            try {
                // Buscar e clicar no dropdown de filtro
                const filterFound = await page.evaluate(() => {
                    // Tentar encontrar dropdown/select de serviço
                    const selectors = [
                    ...
                    for (const sel of selectors) {
                        ...
                        if (el) {
                            el.click();
                            return { found: true, type: 'selector', tag: el.tagName };
                        }
                    }
                    ...
                    for (const el of allElements) {
                        ...
                        if (rect.width > 80 && rect.height > 20 && rect.y > 200 && rect.y < 600) {
                            if (text.includes('all service') || 
                                text.includes('filter') || 
                                text.includes('todos os serviço') ||
                                text === 'service' ||
                                text === 'serviço') {
                                el.click();
                                return { found: true, type: 'text', text: text };
                            }
                        }
                    }
                    return { found: false };
                ...
                if (filterFound.found) {
                    console.log(`   ✅ Filtro encontrado (${filterFound.type}: ${filterFound.tag || filterFound.text})`);
                    ...
                    // LISTAR todas as opções disponíveis no dropdown
                        ...
                        selects.forEach(sel => {
                            sel.querySelectorAll('option').forEach(opt => {
                                if (opt.textContent.trim()) {
                                    options.push({ type: 'option', text: opt.textContent.trim() });
                                }
                            });
                        });
                        
                        // Verificar itens de lista (dropdown customizado)
                        const listItems = document.querySelectorAll('li, [role="option"], [class*="dropdown-item"], [class*="option"]');
                        listItems.forEach(li => {
                            const text = (li.innerText || li.textContent || '').trim();
                            const rect = li.getBoundingClientRect();
                            if (text && rect.height > 0 && rect.width > 50 && text.length < 50) {
                                options.push({ type: 'listItem', text: text });
                            }
                        });
                    ...
                    console.log(`   📋 Opções no dropdown (${dropdownOptions.length}):`);
                    dropdownOptions.forEach((opt, i) => {
                        console.log(`      [${i}] ${opt.text}`);
                    });
                    
                    // Buscar e clicar no serviço - BUSCA FLEXÍVEL
                    const serviceLower = service.toLowerCase();
                    const serviceClicked = await page.evaluate((targetService) => {
                        const targetLower = targetService.toLowerCase();
                        
                        // Verificar select options primeiro
                        const selects = document.querySelectorAll('select');
                        for (const sel of selects) {
                            for (const opt of sel.options) {
                                const optText = opt.textContent.trim().toLowerCase();
                                if (optText.includes(targetLower) || targetLower.includes(optText.replace(' service', ''))) {
                                    sel.value = opt.value;
                                    sel.dispatchEvent(new Event('change', { bubbles: true }));
                                    return { clicked: true, method: 'select', text: opt.textContent };
                                }
                            }
                        }
                        
                        // Verificar itens de lista
                        const listItems = document.querySelectorAll('li, [role="option"], [class*="dropdown-item"], [class*="option"], div, span');
                        for (const li of listItems) {
                            const text = (li.innerText || li.textContent || '').trim();
                            const textLower = text.toLowerCase();
                            const rect = li.getBoundingClientRect();
                            
                            if (rect.height > 0 && rect.width > 50 && rect.y > 100) {
                                // Busca flexível
                                if (textLower === targetLower ||
                                    textLower === targetLower + ' service' ||
                                    textLower.includes(targetLower) ||
                                    targetLower.includes(textLower.replace(' service', ''))) {
                                    li.click();
                                    return { clicked: true, method: 'listItem', text: text };
                                }
                            }
                        }
                    ...
                    if (serviceClicked.clicked) {
                        console.log(`   ✅ Serviço selecionado: "${serviceClicked.text}" (via ${serviceClicked.method})`);
                        await new Promise(r => setTimeout(r, 1500));
                    } else {
                        console.log(`   ⚠️ Serviço "${service}" não encontrado no dropdown`);
                    }
                } else {
                    console.log('   ⚠️ Dropdown de filtro não encontrado');
                }
