    ...
    try {
        ...
        if (data.results.length > 0) {
            ...
            for (let i = 0; i < Math.min(data.results.length, 6); i++) {
                ...
                try {
                    // PASSO 1: Fechar TODOS os painéis abertos primeiro
                    await page.evaluate(() => {
                        // Procurar botões de fechar (chevron para cima ou painéis expandidos)
                        document.querySelectorAll('[class*="expand"], [class*="chevron"], [aria-expanded="true"]').forEach(el => {
                            if (el.getAttribute('aria-expanded') === 'true' || el.closest('[aria-expanded="true"]')) {
                                el.click();
                            }
                        });
                    });
                    ...
                    // PASSO 2: Scroll para a posição correta
                    await page.evaluate((index) => {
                        // Encontrar todas as linhas de resultado
                        const rows = [];
                        document.querySelectorAll('div, article, section').forEach(el => {
                            ...
                            if (text.includes('MSC') && text.includes('Days') &&
                                ...
                                rows.push({ el, top: rect.top + window.scrollY });
                            }
                        });
                        ...
                        if (index < uniqueRows.length) {
                            uniqueRows[index].el.scrollIntoView({ block: 'center', behavior: 'instant' });
                        }
                    }, i);
                    await new Promise(r => setTimeout(r, 500));
                    
                    // PASSO 3: Clicar na linha correta
                    const clicked = await page.evaluate(async (index, vesselName) => {
                        ...
                        if (index >= uniqueRows.length) {
                            return { success: false, error: `Índice ${index} >= ${uniqueRows.length} linhas visíveis` };
                        }
                        
                        const targetRow = uniqueRows[index];
                        
                        // Procurar botão de expandir
                    ...
                    if (clicked && clicked.success) {
                        ...
                        // PASSO 4: Aguardar expansão do painel (mais tempo)
