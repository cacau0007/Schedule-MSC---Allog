            ...
            for (let i = 0; i < Math.min(data.results.length, 6); i++) {
                ...
                try {
                    ...
                    if (clicked && clicked.success) {
                        ...
                        const details = await page.evaluate((vesselName, currentService, polName, podName) => {
                            ...
                            // Códigos de terminais para IGNORAR (não são portos)
                            const terminalCodes = ['PNIT', 'TUAS', 'JICT', 'APM', 'PSA', 'TERMINAL'];
                            ...
                            for (let i = 0; i < lines.length; i++) {
                                ...
                                // Procurar código de porto entre parênteses
                                // Formato: "Nome (CODIGO)" - código deve ter 5-6 caracteres e começar com 2 letras (país)
                                const portMatch = line.match(/^([A-Za-z][A-Za-z\s,\-\.]+?)\s*\(([A-Z]{2}[A-Z]{2,4})\)/);
                                if (portMatch) {
                                    const portName = portMatch[1].trim();
                                    const portCode = portMatch[2];
                                    
                                    // Ignorar se for código de terminal
                                    const isTerminal = terminalCodes.some(t => portCode.includes(t) || portName.toUpperCase().includes('TERMINAL'));
                                    if (isTerminal) continue;
                                    
                                    // Extrair últimas 3 letras do código (ex: SGSIN -> SIN, KRPUS -> PUS)
                                    const shortCode = portCode.length >= 5 ? portCode.substring(2) : portCode;
                                    
                                    // Verificar se tem serviço principal associado (nas próximas linhas)
                                    let portService = '-';
                                    for (let j = i; j <= Math.min(lines.length - 1, i + 5); j++) {
                                        for (const svc of mainServices) {
                                            if (lines[j].includes(svc + ' Service') || lines[j] === svc + ' Service' || lines[j] === svc) {
                                                portService = svc;
                                                break;
                                            }
                                        }
                                    ...
                                    // Buscar data de chegada (primeira data na linha ou próximas)
                                    let portDate = '';
                            ...
                            // Procurar serviço nas linhas (se ainda não achou)
                            if (service === '-') {
                                for (const line of lines) {
                                    for (const svc of mainServices) {
                                        if (line.includes(svc) && !line.includes('Filter') && !line.includes('All Services')) {
                                            service = svc;
                                            break;
                                        }
                                    }
                            ...
                            // Analisar portos encontrados (mínimo 2: POL e POD)
                            if (portsFound.length >= 3) {
                                // Tem transbordo - PRIORIDADE: porto com serviço principal
                                let transbordoPort = null;
                                
                                // Primeiro: procurar porto com serviço Santana/Carioca/Ipanema/Jade
                                for (let p = 1; p < portsFound.length - 1; p++) {
                                    if (portsFound[p].service !== '-') {
                                        transbordoPort = portsFound[p];
                                        break;
                                    }
                                }
                                
                                // Se não achou, pegar o penúltimo (último antes do POD)
                                if (!transbordoPort) {
                                    transbordoPort = portsFound[portsFound.length - 2];
                                }
                                
                                transbordo = transbordoPort.code;
                                transbordoDate = transbordoPort.date;
                                
                                // Usar serviço do porto se ainda não definido
                                if (service === '-' && transbordoPort.service !== '-') {
                                    service = transbordoPort.service;
                                }
                            }
                            
                            return { service, transbordo, transbordoDate, portsCount: portsFound.length };
                        }, item.vessel, item.service, pol, pod);
