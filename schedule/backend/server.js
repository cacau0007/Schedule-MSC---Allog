...
const SERVICE_ROUTES = {
    ...
    // ========================================================
    // MANAUS - Via T/S Santos (Santana principal, Ipanema backup, Carioca via SIN)
    // ========================================================
    'Qingdao-Manaus': ['Santana'],
    'Shanghai-Manaus': ['Santana', 'Ipanema'],
    'Ningbo-Manaus': ['Santana', 'Ipanema'],
    'Busan-Manaus': ['Santana', 'Ipanema'],
    'Shekou-Manaus': ['Santana'],
    'Singapore-Manaus': ['Santana', 'Carioca'],
    'Yantian-Manaus': ['Ipanema'],
    'Hong Kong-Manaus': ['Ipanema'],
    
    // ========================================================
    // VITÓRIA - Via T/S Rio de Janeiro (Carioca ou Santana)
    // ========================================================
    ...
    // ========================================================
    // PECÉM - Via T/S Santos ou Salvador (Santana)
    // ========================================================
...
// PODs que são atendidos via CONEXÃO/FEEDER - têm serviços específicos via T/S
// NÃO mais tratados como "todos os serviços"
const CONNECTION_PODS = [
    'Fortaleza', 'Belem'  // Apenas esses realmente não têm mapeamento claro
];

// Função para verificar se é POD de conexão sem mapeamento
function isConnectionPod(pod) {
    return CONNECTION_PODS.includes(pod);
}
...
// - POD é de conexão (não filtrar)
function getAvailableServices(pol, pod) {
    // Se for rota de conexão (POL ou POD), retornar null (não filtrar)
    if (isConnectionRoute(pol) || isConnectionPod(pod)) {
        return null;
    }
...
app.get('/api/available-services', (req, res) => {
    ...
    // Verificar se é rota de conexão (POL ou POD)
    if (isConnectionRoute(pol) || isConnectionPod(pod)) {
        return res.json({ 
            services: null,
            mapped: false,
            isConnection: true,
            message: `Rota de conexão - busca sem filtro`
        });
    }
...
async function scrapeMSC(pol, pod, service = null) {
    ...
    // Verificar se é rota de conexão (POL ou POD)
    const isConnection = isConnectionRoute(pol) || isConnectionPod(pod);
    if (isConnection) {
    ...
    try {
        ...
        if (isConnection && basicData.length > 0) {
            ...
            await page.screenshot({ path: path.join(EXPORTS_DIR, 'msc-08-antes-clique.png') });
            
            for (let i = 0; i < Math.min(basicData.length, 30); i++) { // Processar até 30 resultados
                const item = basicData[i];
                console.log(`   [${i+1}/${Math.min(basicData.length, 30)}] ${item.vessel}...`);
                
                try {
                    // Estratégia simplificada: clicar diretamente no texto do navio
                    const clicked = await page.evaluate((vesselName) => {
                        // Procurar pelo texto do navio na página
                        ...
                        while (node = walker.nextNode()) {
                            if (node.textContent.includes(vesselName)) {
                                // Encontrou o texto, subir para o elemento pai clicável
                                let el = node.parentElement;
                                
                                // Subir até encontrar um elemento com tamanho adequado (a linha)
                                while (el && el.tagName !== 'BODY') {
                                    const rect = el.getBoundingClientRect();
                                    // Linha tem largura grande e altura média
                                    if (rect.width > 600 && rect.height > 40 && rect.height < 150) {
                                        el.click();
                                        return { clicked: true, method: 'row-by-vessel', tag: el.tagName };
                                    }
                                    el = el.parentElement;
                                }
                                
                                // Se não achou linha, clicar no próprio elemento do navio
                                if (node.parentElement) {
                                    node.parentElement.click();
                                    return { clicked: true, method: 'vessel-direct' };
                                }
                            }
                        }
                    ...
                    if (clicked.clicked) {
                        console.log(`      Clicado via: ${clicked.method}`);
                        await new Promise(r => setTimeout(r, 2500)); // Esperar expansão
                        
                        // Tirar screenshot após clique
                        await page.screenshot({ path: path.join(EXPORTS_DIR, `msc-09-apos-clique-${i}.png`) });
                        
                        // Extrair detalhes da área expandida
                        const details = await page.evaluate((vesselName) => {
                            ...
                            // Serviços conhecidos
                            const knownServices = ['Santana', 'Carioca', 'Ipanema', 'Jade', 'Seagull', 'Seahorse', 'Tiger', 'Dragon', 'Lion', 'Phoenix'];
                            
                            // Portos de transbordo conhecidos
                            const transhipmentPorts = {
                                'Singapore': ['Singapore', 'SGSIN', '(SG)'],
                                'Busan': ['Busan', 'KRPUS', '(KR)'],
                                'Tanjung Pelepas': ['Tanjung Pelepas', 'MYTPP'],
                                'Port Klang': ['Port Klang', 'MYPKG'],
                                'Colombo': ['Colombo', 'LKCMB'],
                                'Sines': ['Sines', 'PTSIE']
                            };
                            
                            // Regex para datas no formato "Mon 28th Jan 2026" ou "28th Jan 2026" ou "28 Jan 2026"
                            const dateRegex = /(?:Mon|Tue|Wed|Thu|Fri|Sat|Sun)?\s*(\d{1,2})(?:st|nd|rd|th)?\s+(Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)\s+(\d{4})/gi;
                            ...
                            for (let i = 0; i < lines.length; i++) {
                                ...
                                // Detectar serviço sem "Service" na linha
                                if (service === '-') {
                                    for (const svc of knownServices) {
                                        if (line === svc || line.startsWith(svc + ' ')) {
                                            service = svc;
                                            break;
                                        }
                                    }
                                ...
                                // Detectar porto de transbordo E data na mesma linha ou linhas próximas
                                for (const [portName, identifiers] of Object.entries(transhipmentPorts)) {
                                    for (const id of identifiers) {
                                        if (line.includes(id)) {
                                            transhipmentPort = portName;
                                            
                                            // Procurar data na mesma linha
                                            const dateMatch = line.match(dateRegex);
                                            if (dateMatch && dateMatch.length > 0) {
                                                transhipmentDate = dateMatch[0];
                                            } else {
                                                // Procurar nas linhas anteriores (até 2 linhas)
                                                for (let j = Math.max(0, i - 2); j < i; j++) {
                                                    const prevMatch = lines[j].match(dateRegex);
                                                    if (prevMatch) {
                                                        transhipmentDate = prevMatch[prevMatch.length - 1]; // Última data encontrada
                                                    }
                                                }
                                                
                                                // Procurar nas linhas seguintes (até 2 linhas)
                                                if (!transhipmentDate) {
                                                    for (let j = i + 1; j <= Math.min(lines.length - 1, i + 2); j++) {
                                                        const nextMatch = lines[j].match(dateRegex);
                                                        if (nextMatch) {
                                                            transhipmentDate = nextMatch[0]; // Primeira data encontrada
                                                            break;
                                                        }
                                                    }
                                                }
                        ...
                        // Tentar fechar/colapsar
                        await page.keyboard.press('Escape');
                        await new Promise(r => setTimeout(r, 500));
                    } else {
                        console.log(`      ⚠️ Não conseguiu clicar`);
                    }
        ...
        // Para rotas DIRETAS: remover duplicados por navio
        let finalResults;
        
        if (isConnection) {
            // Manter todos - mesmo navio pode ter múltiplas saídas com serviços diferentes
            finalResults = detailedResults;
            console.log(`   Mantendo todos: ${finalResults.length} resultados (rota de conexão)`);
        } else {
            ...
            finalResults = Array.from(seenVessels.values());
            console.log(`   Únicos: ${finalResults.length} navios`);
        }
...
// ============================================
// EXCEL - Geração com nova estrutura de colunas
// ============================================
async function generateExcel(sailings, pol, pod, filename) {
    console.log(`📊 Gerando Excel: ${filename} (${sailings.length} registros)`);
    
    const workbook = new ExcelJS.Workbook();
    const sheet = workbook.addWorksheet('SCHEDULES');
    
    // Abreviações dos portos
    const getAbbrev = (port) => {
        const abbrevs = {
            'Shanghai': 'SHA', 'Ningbo': 'NGB', 'Qingdao': 'TAO', 'Yantian': 'YTN',
            'Shekou': 'SHE', 'Hong Kong': 'HKG', 'Xiamen': 'XMN', 'Nansha': 'NSA',
            'Busan': 'BUS', 'Singapore': 'SIN', 'Jakarta': 'JKT', 'Surabaya': 'SRB',
            'Ho Chi Minh': 'SGN', 'Haiphong': 'HPH', 'Laem Chabang': 'LCB',
            'Port Klang': 'PKL', 'Tanjung Pelepas': 'TPP', 'Colombo': 'CMB',
            'Santos': 'SSZ', 'Navegantes': 'NVT', 'Itapoa': 'ITP', 'Paranagua': 'PNG',
            'Rio de Janeiro': 'RIO', 'Itaguai': 'ITG', 'Imbituba': 'IMB', 'Itajai': 'ITJ',
            'Rio Grande': 'RIG', 'Montevideo': 'MVD', 'Buenos Aires': 'BUE',
            'Salvador': 'SSA', 'Suape': 'SUA', 'Vitoria': 'VIX', 'Manaus': 'MAO'
        };
        return abbrevs[port] || (port ? port.substring(0, 3).toUpperCase() : '-');
    };
    
    const polAbbrev = getAbbrev(pol);
    const podAbbrev = getAbbrev(pod);
    
    // Título
    sheet.mergeCells('A1:H1');
    const titleCell = sheet.getCell('A1');
    titleCell.value = `ALLOG - Schedules: ${pol} (${polAbbrev}) - ${pod} (${podAbbrev})`;
    titleCell.font = { bold: true, size: 14 };
    titleCell.alignment = { horizontal: 'center' };
    
    // Headers com abreviações
    const headers = ['SERVICO', 'NAVIO', `ETD (${polAbbrev})`, 'TRANSBORDO', 'DATA TRANS', `ETA (${podAbbrev})`, 'TRANSIT', 'TIPO'];
    sheet.addRow(headers);
    
    // Sanitizar string
    const safeString = (val) => {
        if (val === null || val === undefined) return '-';
        return String(val).replace(/[^\x20-\x7E\xA0-\xFF]/g, '').substring(0, 200);
    };
    
    // Dados
    sailings.forEach(s => {
        const transAbbrev = s.transbordo && s.transbordo !== '-' ? getAbbrev(s.transbordo) : '-';
        ...
        sheet.addRow([
            ...
            safeString(s.etd),
            transAbbrev,
            safeString(s.transbordoDate),
            safeString(s.eta),
            safeString(s.transit || s.transitTime),
            safeString(s.routeType)
        ]);
    });
    ...
    try {
        ...
        console.error(`❌ Erro ao salvar Excel: ${err.message}`);
        ...
        try {
            const csvContent = [
                ...
                ...sailings.map(s => [
                    ...
                    s.vessel || '-',
                    s.etd || '-',
                    s.transbordo ? getAbbrev(s.transbordo) : '-',
                    s.transbordoDate || '-',
                    s.eta || '-',
                    s.transit || '-',
                    s.routeType || '-'
            ...
            console.log(`✅ CSV salvo como fallback: ${csvPath}`);
        } catch (csvErr) {
            console.error(`❌ Erro ao salvar CSV: ${csvErr.message}`);
        }
    }
}
