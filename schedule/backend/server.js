...
app.post('/api/export', async (req, res) => {
    ...
    try {
        ...
        sheet.getCell('F1').value = servicesText;
        sheet.getCell('F1').font = { bold: true, size: 10 };
        sheet.getCell('F1').fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FFFFFF00' } };
                ...
                try {
                    ...
                    if (clicked && clicked.success) {
                        ...
                        const details = await page.evaluate((vesselName, currentService, polName, podName) => {
                            ...
                            // Cada porto aparece com código entre parênteses: "Valencia (ESVLC)"
                            
                            let foundPOL = false;
                            let foundTransbordo = false;
                            ...
                            for (let i = 0; i < lines.length; i++) {
                                ...
                                // Procurar código de porto entre parênteses - formato: "Nome (CODIGO)"
                                const portMatch = line.match(/^([A-Za-z\s]+)\s*\(([A-Z]{5})\)/);
                                if (portMatch) {
                                    const portName = portMatch[1].trim();
                                    const portCode = portMatch[2]; // Ex: CNSHA, ESVLC, BRNVT
                                    
                                    // Extrair apenas as 3 últimas letras do código
                                    const shortCode = portCode.substring(2); // SHA, VLC, NVT
                                    
                                    // Verificar se é POL (primeiro porto)
                                    if (!foundPOL) {
                                        foundPOL = true;
                                        continue; // Pular o POL
                                    }
                                    
                                    // Verificar se é POD (último porto - contém o nome do destino)
                                    if (podName && portName.toLowerCase().includes(podName.toLowerCase().substring(0, 4))) {
                                        continue; // Pular o POD
                                    }
                                    
                                    // Se chegou aqui, é transbordo!
                                    if (!foundTransbordo && transbordo === '-') {
                                        transbordo = shortCode;
                                        foundTransbordo = true;
                                        
                                        // Buscar data de chegada (Estimated Time of Arrival) nas linhas próximas
