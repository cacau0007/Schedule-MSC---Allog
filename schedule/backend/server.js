            ...
            for (let i = 0; i < Math.min(data.results.length, 15); i++) {
                ...
                try {
                    ...
                    if (clicked && clicked.success) {
                        ...
                        // Extrair dados APENAS do painel expandido (não do body inteiro)
                        // Procurar pela seção que contém "Port of Discharge/Load" ou "Singapore"
                        const details = await page.evaluate((vesselName) => {
                            let service = '-';
                            let transbordo = '-';
                            let transbordoDate = '';
                            
                            // Procurar pelo painel expandido que contém a tabela de portos
                            // Ele tem "Port of Discharge/Load", "Singapore", etc.
                            const allText = document.body.innerText;
                            
                            // Verificar se Singapore está visível (indica que expandiu)
                            if (!allText.includes('Singapore') && !allText.includes('SGSIN') && 
                                !allText.includes('Busan') && !allText.includes('KRPUS')) {
                                return { service: '-', transbordo: '-', transbordoDate: '', expanded: false };
                            }
                            
                            const lines = allText.split('\n').map(l => l.trim()).filter(l => l);
                            
                            // Procurar a linha que contém Singapore ou Busan
                            for (let i = 0; i < lines.length; i++) {
                                const line = lines[i];
                                ...
                                if ((lower.includes('singapore') || line.includes('SGSIN')) && transbordo === '-') {
                                    ...
                                    // Procurar serviço e data nas linhas próximas
                                    for (let j = Math.max(0, i - 5); j <= Math.min(lines.length - 1, i + 5); j++) {
                                        const nearLine = lines[j];
                                        
                                        // Serviço
                                        if (nearLine.includes('Ipanema') && service === '-') service = 'Ipanema';
                                        else if (nearLine.includes('Santana') && service === '-') service = 'Santana';
                                        else if (nearLine.includes('Carioca') && service === '-') service = 'Carioca';
                                        else if (nearLine.includes('Jade') && service === '-') service = 'Jade';
                                        
                                        // Data (formato: "Tue 27th Jan 2026")
                                        if (!transbordoDate) {
                                            const dateMatch = nearLine.match(/(\w{3}\s+\d{1,2}(?:st|nd|rd|th)?\s+\w{3}\s+\d{4})/);
                                            if (dateMatch) transbordoDate = dateMatch[1];
                                        }
                                    }
                                    break; // Encontrou Singapore, pode parar
                                }
                            ...
                            return { service, transbordo, transbordoDate, expanded: true };
                        }, item.vessel);
                        ...
                        if (details.expanded) {
                            ...
                            console.log(`      ⚠️ Painel não expandiu`);
                        }
                        
                        // IMPORTANTE: Fechar o painel antes de ir para o próximo
                        // Clicar na mesma linha novamente para fechar
