                ...
                try {
                    ...
                    if (clicked && clicked.success) {
                        ...
                        const details = await page.evaluate((vesselName, currentService, polName, podName) => {
                            ...
                            // Procurar no painel expandido
                            // Estrutura pode ter múltiplos transbordos: POL -> T1 -> T2 -> POD
                            // Queremos o ÚLTIMO transbordo (penúltimo porto)
                            ...
                            for (let i = 0; i < lines.length; i++) {
                                ...
                                // Procurar serviço principal nas linhas
                                if (service === '-') {
                                ...
                                // Formatos: "Nome (CODIGO)" ou "Nome, Local (CODIGO)"
                                const portMatches = line.match(/([A-Za-z][A-Za-z\s,\-\.]+)\s*\(([A-Z]{2}[A-Z0-9]{2,4})\)/g);
                                if (portMatches) {
                                    for (const match of portMatches) {
                                        const parts = match.match(/([A-Za-z][A-Za-z\s,\-\.]+)\s*\(([A-Z]{2}[A-Z0-9]{2,4})\)/);
                                        if (parts) {
                                            const portName = parts[1].trim();
                                            const portCode = parts[2];
                                            // Extrair últimas 3 letras do código (ex: SGSIN -> SIN, KRPUS -> PUS)
                                            const shortCode = portCode.length >= 5 ? portCode.substring(2) : portCode;
                                            
                                            // Verificar se tem serviço principal associado
                                            let portService = '-';
                                            for (let j = i; j <= Math.min(lines.length - 1, i + 2); j++) {
                                                for (const svc of mainServices) {
                                                    if (lines[j].includes(svc + ' Service') || lines[j] === svc) {
                                                        portService = svc;
                                                        break;
                                                    }
                                                }
                                            ...
                                            // Buscar data próxima (hora estimada de chegada)
                                            let portDate = '';
                                            for (let j = i; j <= Math.min(lines.length - 1, i + 2); j++) {
                            ...
                            // Primeiro = POL, Último = POD, Penúltimo = Transbordo principal
                            if (portsFound.length >= 3) {
                                // Tem transbordo - pegar o PENÚLTIMO porto (último antes do POD)
                                const transbordoPort = portsFound[portsFound.length - 2];
                                transbordo = transbordoPort.code;
                                transbordoDate = transbordoPort.date;
                                
                                // Se o serviço ainda não foi definido, usar o do porto de transbordo principal
                                if (service === '-' && transbordoPort.service !== '-') {
                                    service = transbordoPort.service;
                                }
                            } else if (portsFound.length === 2) {
                                // Rota direta (só POL e POD)
                                transbordo = '-';
                            }
