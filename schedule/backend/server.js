                ...
                try {
                    ...
                    if (clicked && clicked.success) {
                        ...
                        const details = await page.evaluate((vesselName) => {
                            ...
                            // Verificar se expandiu (tem portos de transbordo visíveis)
                            ...
                            // Procurar TODOS os serviços mencionados
                            // O serviço principal é Santana, Carioca, Ipanema ou Jade
                            // Feeders são: Seagull, Pertiwi, Sambar, etc
                            const mainServices = ['Santana', 'Carioca', 'Ipanema', 'Jade'];
                            
                            for (let i = 0; i < lines.length; i++) {
                                const line = lines[i];
                                
                                // Procurar serviço principal
                                ...
                                // Detectar primeiro porto de transbordo e sua data
                                if (transbordo === '-') {
                                    if (line.includes('Singapore') || line.includes('SGSIN')) {
                                        transbordo = 'SIN';
                                    } else if (line.includes('Busan') || line.includes('KRPUS')) {
                                        transbordo = 'BUS';
                                    } else if (line.includes('Tanjung Pelepas') || line.includes('MYTPP')) {
                                        transbordo = 'TPP';
                                    }
                                    
                                    // Se achou transbordo, pegar data próxima
...
function formatDate(dateStr) {
    ...
    try {
        // Entrada: "Mon 13th Jan 2025" ou "Sat 17th Jan 2026" ou "Sun 25th Jan 2026"
        // Saída: "13/01/2025" (sem dia da semana)
        
        const match = dateStr.match(/(\d{1,2})(?:st|nd|rd|th)?\s+(\w{3})\s+(\d{4})/);
