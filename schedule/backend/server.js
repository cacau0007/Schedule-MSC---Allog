    ...
    try {
        ...
        if (data.results.length > 0) {
            ...
            for (let i = 0; i < Math.min(data.results.length, 6); i++) {
                ...
                try {
                    // PASSO 1: Fechar qualquer painel aberto
                    await page.keyboard.press('Escape');
                    await new Promise(r => setTimeout(r, 500));
                    
                    // PASSO 2: Scroll para o topo da página primeiro
                    await page.evaluate(() => {
                        window.scrollTo(0, 0);
                    });
                    await new Promise(r => setTimeout(r, 300));
                    
                    // PASSO 3: Encontrar TODAS as linhas de resultado (não só visíveis)
                    const allRows = await page.evaluate(() => {
                        ...
                        document.querySelectorAll('div, article, section').forEach(el => {
                            ...
                            if (text.includes('MSC') && text.includes('Days') &&
                                ...
                                rect.width > 800 && rect.height > 50 && rect.height < 150) {
                                rows.push({ 
                    ...
                    await page.evaluate((scrollTo) => {
                        window.scrollTo(0, Math.max(0, scrollTo - 200));
                    }, targetTop);
                    await new Promise(r => setTimeout(r, 500));
                    
                    // PASSO 5: Clicar na linha correta (agora deve estar visível)
                    const clicked = await page.evaluate((targetIndex, targetText) => {
                        const rows = [];
                        document.querySelectorAll('div, article, section').forEach(el => {
                            const text = el.innerText || '';
                            const rect = el.getBoundingClientRect();
                            if (text.includes('MSC') && text.includes('Days') &&
                                ...
                                rows.push({ el, top: rect.top, text: text.substring(0, 80) });
                            }
                        });
                        
                        rows.sort((a, b) => a.top - b.top);
                        
                        // Encontrar a linha que mais se aproxima do texto esperado
                        let targetRow = null;
                        for (const row of rows) {
                            if (row.text.includes(targetText.substring(0, 30))) {
                                targetRow = row;
                                break;
                            }
                        }
                        
                        // Se não encontrou pelo texto, pegar a primeira visível
                        if (!targetRow && rows.length > 0) {
                            targetRow = rows[0];
                        }
                        
                        if (!targetRow) {
                            return { success: false, error: 'Nenhuma linha visível' };
                        }
                        ...
                        return { success: true, rowText: targetRow.text.substring(0, 50), rowTop: targetRow.top };
                    }, i, allRows[i].text);
                    
                    if (clicked && clicked.success) {
                        console.log(`      Clicou na linha ${i+1} (top: ${clicked.rowTop})`);
                        
                        // PASSO 6: Aguardar expansão do painel
                        ...
                        if (i === data.results.length - 1 || i === 5) {
                            await page.screenshot({ path: path.join(EXPORTS_DIR, 'msc-09-ultimo-expandido.png') });
                        }
                        ...
                        const details = await page.evaluate((vesselName, currentService, polName, podName, expectedETA) => {
                            ...
                            if (portsFound.length >= 2) {
                                // PRIORIDADE 1: Porto com serviço principal (Santana/Carioca/Ipanema/Jade)
