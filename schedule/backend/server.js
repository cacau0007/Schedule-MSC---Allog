...
// Se a rota não estiver mapeada, retorna null (mostrar todos)
// ============================================

// Lista de todos os serviços MSC Asia-Brasil
const ALL_SERVICES = ['Santana', 'Carioca', 'Ipanema', 'Jade', 'Tiger'];

const SERVICE_ROUTES = {
    // ========== SANTANA - Nordeste Brasil (DIRETO) ==========
    // Rotação: Yantian – Ningbo – Shanghai – Qingdao – Busan – Cristobal – Suape – Salvador
    ...
    // ========== CARIOCA - Sul/Sudeste Brasil (DIRETO) ==========
    // Rotação atualizada: Qingdao-Busan-Ningbo-Shanghai-Shekou-Singapore-Colombo-Rio-Santos-Paranagua-Navegantes-Imbituba-Itajai
    ...
    // ========== NAVEGANTES - Múltiplos serviços ==========
    // Baseado em verificação real no site MSC (Jan/2025)
    'Shanghai-Navegantes': ['Ipanema', 'Santana', 'Jade', 'Carioca'],
    'Ningbo-Navegantes': ['Ipanema', 'Carioca', 'Jade', 'Santana'],
    'Busan-Navegantes': ['Ipanema', 'Carioca', 'Santana'],
    'Qingdao-Navegantes': ['Jade', 'Santana'],  // Verificado no site - só Jade e Santana
    'Shekou-Navegantes': ['Ipanema', 'Carioca'],
    'Hong Kong-Navegantes': ['Ipanema'],
    'Singapore-Navegantes': ['Ipanema'],
    'Yantian-Navegantes': ['Carioca', 'Santana'],
    'Xiamen-Navegantes': ['Jade', 'Tiger'],
    'Tianjin-Navegantes': ['Jade', 'Tiger', 'Santana'],
    
    // ========== IPANEMA - Sul Brasil + Argentina/Uruguai ==========
    // Rotação: Busan-Shanghai-Ningbo-Shekou-HongKong-Singapore-Santos-Paranagua-Navegantes-Montevideo-BuenosAires-RioGrande
...
function getAvailableServices(pol, pod) {
    ...
    return SERVICE_ROUTES[routeKey] || ALL_SERVICES; // Se não mapeado, retorna todos
}
...
app.get('/api/available-services', (req, res) => {
    ...
    return res.json({ 
        services: services,
        mapped: isMapped,
        message: isMapped ? `Serviços disponíveis para ${pol}-${pod}` : 'Rota não mapeada - mostrando todos os serviços'
    });
});
...
async function scrapeMSC(pol, pod, service = null) {
    ...
    try {
        ...
        if (service && service !== 'ALL') {
            ...
            try {
                // PASSO 1: Encontrar o botão "Filter by" com múltiplas estratégias
                const filterInfo = await page.evaluate(() => {
                    // Estratégia 1: Buscar por texto exato
                    const allElements = document.querySelectorAll('*');
                    
                    // Lista de padrões para encontrar o botão de filtro
                    const patterns = [
                        'Filter by: All Services',
                        'Filter by:All Services', 
                        'Filter by: All',
                        'All Services'
                    ];
                    ...
                    for (const el of allElements) {
                        ...
                        const matchesPattern = patterns.some(p => text.includes(p)) ||
                                              (text.includes('Filter') && text.includes('Service'));
                        
                        if (matchesPattern) {
                            const rect = el.getBoundingClientRect();
                            // Validar tamanho (botão de dropdown típico)
                            if (rect.width > 80 && rect.width < 400 && 
                    ...
                    // Estratégia 2: Buscar por select ou dropdown com "Service"
                    const selects = document.querySelectorAll('select, [role="listbox"], [role="combobox"]');
                    for (const sel of selects) {
                        const text = (sel.innerText || sel.textContent || '').toLowerCase();
                        if (text.includes('service')) {
                            const rect = sel.getBoundingClientRect();
                            if (rect.y > 200) {
                                return {
                                    found: true,
                                    text: 'Service dropdown',
                                    x: rect.x + rect.width / 2,
                                    y: rect.y + rect.height / 2,
                                    width: rect.width,
                                    height: rect.height,
                ...
                if (filterInfo.found) {
                    ...
                    // Esperar dropdown abrir (tempo maior para garantir)
                    await new Promise(r => setTimeout(r, 3000));
                    
                    await page.screenshot({ path: path.join(EXPORTS_DIR, 'msc-06-dropdown-aberto.png') });
                    
                    // NOVA ESTRATÉGIA: Buscar opções do dropdown de forma mais robusta
                    const dropdownBottom = filterInfo.bottom || filterInfo.y + 30;
                    
                    const options = await page.evaluate((btnBottom) => {
                        const items = [];
                        const knownServices = ['santana', 'carioca', 'ipanema', 'jade', 'tiger', 'dragon', 'lion', 'phoenix', 'all services'];
                        
                        // Buscar todos os elementos que podem ser opções de dropdown
                        const allElements = document.querySelectorAll('*');
                        ...
                        allElements.forEach(el => {
                            ...
                            // Ignorar elementos muito grandes ou muito pequenos
                            if (rect.height < 10 || rect.height > 60) return;
                            if (rect.width < 50 || rect.width > 400) return;
                            
                            // Deve estar abaixo ou próximo do botão
                            if (rect.y < btnBottom - 50) return;
                            if (rect.y > btnBottom + 400) return;
                            
                            // Verificar se é um serviço conhecido
                            let matchedService = null;
                            for (const svc of knownServices) {
                                if (textLower === svc || 
                                    textLower === svc + ' service' ||
                                    textLower.includes(svc)) {
                                    matchedService = svc;
                                    break;
                                }
                            }
                            ...
                            if (matchedService) {
                                ...
                                const alreadyAdded = items.some(i => i.textLower === textLower);
                                if (!alreadyAdded && !text.includes('Filter by')) {
                                    items.push({
                                        text: text,
                                        textLower: textLower,
                    ...
                    if (options.length > 0) {
                        ...
                        let match = options.find(opt => 
                            opt.service === targetService ||
                            opt.textLower === targetService ||
                            opt.textLower === targetService + ' service'
                        );
                        
                        // Se não encontrou match exato, buscar parcial
                        if (!match) {
                            match = options.find(opt => opt.textLower.includes(targetService));
                        }
                        ...
                        if (match) {
                            ...
                            // Aguardar a página atualizar com o filtro
                            await new Promise(r => setTimeout(r, 2500));
                        } else {
                            console.log(`   ⚠️ Serviço "${service}" não encontrado nas opções disponíveis`);
                            console.log(`      Opções disponíveis: ${options.map(o => o.service).join(', ')}`);
                        }
                    } else {
                        console.log('   ⚠️ Nenhuma opção de serviço encontrada no dropdown');
                        
                        // Tentar capturar screenshot para debug
                        await page.screenshot({ path: path.join(EXPORTS_DIR, 'msc-06b-dropdown-vazio.png'), fullPage: true });
                        
                        // Debug adicional: verificar se há elementos role="option" ou li
                        const debugInfo = await page.evaluate(() => {
                            ...
                            // Buscar elementos com role="option"
                            document.querySelectorAll('[role="option"]').forEach(el => {
                                const rect = el.getBoundingClientRect();
                                if (rect.y > 200) {
                                    info.roleOptions.push({ text: el.innerText?.trim(), y: rect.y });
                                }
                            });
                            
                            // Buscar li dentro de ul visíveis
                            document.querySelectorAll('ul li').forEach(el => {
                                const rect = el.getBoundingClientRect();
                                if (rect.y > 400 && rect.y < 800 && rect.height > 10) {
                                    info.listItems.push({ text: el.innerText?.trim().substring(0, 30), y: rect.y });
                                }
                            });
                            
                            // Buscar divs com classe dropdown
                            document.querySelectorAll('[class*="dropdown"], [class*="menu"], [class*="list"]').forEach(el => {
                                const rect = el.getBoundingClientRect();
                                if (rect.y > 400 && rect.height > 50) {
                                    info.dropdownMenus.push({ 
                        ...
                        console.log(`      role="option": ${debugInfo.roleOptions.length}`);
                        console.log(`      li items: ${debugInfo.listItems.length}`);
                        console.log(`      dropdown menus: ${debugInfo.dropdownMenus.length}`);
                        
                        if (debugInfo.listItems.length > 0) {
                            console.log('      Primeiros li:', debugInfo.listItems.slice(0, 5));
                        }
                    }
                } else {
