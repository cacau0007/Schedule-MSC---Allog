        ...
        if (service && service !== 'ALL') {
            ...
            try {
                ...
                const filterClicked = await page.evaluate(() => {
                    ...
                    // Fallback: buscar por "All Services" sozinho
                    for (const el of allElements) {
                        const text = (el.innerText || el.textContent || '').trim();
                        if (text === 'All Services' || text === 'Filter by: All Services') {
                            el.click();
                            return { clicked: true, text: text };
                        }
                    }
                ...
                if (filterClicked.clicked) {
                    console.log(`   ✅ Dropdown clicado: "${filterClicked.text}"`);
                    ...
                    // Listar opções do dropdown (apenas itens relevantes)
                    const options = await page.evaluate(() => {
                        const items = [];
                        // Buscar itens de lista que parecem ser serviços
                        const elements = document.querySelectorAll('li, [role="option"], [role="menuitem"], div[class*="option"], div[class*="item"]');
                        elements.forEach(el => {
                            const text = (el.innerText || el.textContent || '').trim();
                            const rect = el.getBoundingClientRect();
                            // Filtrar: deve estar visível, ter tamanho razoável, e texto curto (nome de serviço)
                            if (rect.height > 20 && rect.height < 60 && 
                                rect.width > 80 && rect.width < 400 &&
                                rect.y > 400 && rect.y < 800 &&
                                text.length > 2 && text.length < 40 &&
                                !text.includes('@') && !text.includes('+')) {
                                items.push({ text: text, y: rect.y });
                            }
                        });
                    ...
                    const serviceSelected = await page.evaluate((targetService) => {
                        ...
                        // Buscar em itens de lista
                        const elements = document.querySelectorAll('li, [role="option"], [role="menuitem"], div, span');
                        for (const el of elements) {
                            const text = (el.innerText || el.textContent || '').trim();
                            const textLower = text.toLowerCase();
                            const rect = el.getBoundingClientRect();
                            
                            // Deve estar na área do dropdown (y > 400) e ser clicável
                            if (rect.height > 20 && rect.height < 60 && rect.y > 400 && rect.y < 800) {
                    ...
                    console.log('   ⚠️ Botão "Filter by" não encontrado');
                }
