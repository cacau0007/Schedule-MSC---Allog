        ...
        if (service && service !== 'ALL') {
            ...
            try {
                ...
                if (filterInfo.found) {
                    ...
                    const dropdownY = filterInfo.y;
                    const dropdownX = filterInfo.x;
                    
                    const options = await page.evaluate((btnY, btnX) => {
                        const items = [];
                        
                        // Buscar TODOS os elementos que contêm "Service" no texto
                        // e estão na área do dropdown (próximo ao botão)
                        document.querySelectorAll('*').forEach(el => {
                            const text = (el.innerText || el.textContent || '').trim();
                            const rect = el.getBoundingClientRect();
                            
                            // Deve conter "Service" ou ser um nome de serviço conhecido
                            const isServiceText = 
                                ...
                                text.toLowerCase() === 'dragon' ||
                                text.toLowerCase() === 'lion';
                            
                            // Filtros de posição e tamanho:
                            // - Deve estar abaixo ou próximo do botão (y >= btnY - 50)
                            // - Deve estar em uma faixa de X razoável (não muito longe do botão)
                            // - Deve ter altura de item de menu (15-50px)
                            // - Texto deve ser curto (nome de serviço)
                            const inYRange = rect.y >= btnY - 20 && rect.y < btnY + 300;
                            const inXRange = Math.abs(rect.x - btnX) < 200 || rect.x < 400;
                            const goodSize = rect.height > 15 && rect.height < 55 && rect.width > 50 && rect.width < 350;
                            const shortText = text.length > 3 && text.length < 30;
                            const noNewlines = !text.includes('\n');
                            
                            if (isServiceText && inYRange && goodSize && shortText && noNewlines) {
                                // Evitar duplicatas - pegar o elemento mais interno
                                const alreadyAdded = items.some(i => i.text === text);
                                if (!alreadyAdded) {
                                    items.push({ 
                                        text: text, 
                                        y: Math.round(rect.y),
                        ...
                        // Ordenar por Y e remover o botão original (que também contém "Service")
                        return items
                            .sort((a, b) => a.y - b.y)
                            .filter(i => !i.text.includes('Filter by')); // Remover o botão
                    }, dropdownY, dropdownX);
